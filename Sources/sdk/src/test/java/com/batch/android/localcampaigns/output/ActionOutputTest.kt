...
class ActionOutputTest {
    ...
    fun testExecutesAction() {
        ...
        PowerMockito.doReturn(context).`when`(runtimeManager).getContext()
        ...
        var output = ActionOutput(JSONObject().apply { put("action", "foo") })
        ...
        output =
            ActionOutput(
                JSONObject().apply {
                    put("action", "foobaz")
                    put("args", JSONObject().apply { put("arg1", "val1") })
                }
            )
        Assert.assertTrue(output.displayMessage(fakeCampaign))
        ...
        Mockito.verify(actionModuleSpy)
            .performAction(
                Mockito.eq(context),
                Mockito.eq("foobaz"),
                JSONObjectMockitoMatcher.eq(JSONObject().apply { put("arg1", "val1") }),
                Mockito.isNull(),
            )
    }
    ...
    fun testDoesNotCrashWithInvalidPayload() {
        ...
        val runtimeManager = DITestUtils.mockSingletonDependency(RuntimeManager::class.java, null)
        PowerMockito.doReturn(null).`when`(runtimeManager).getContext()
        ...
        output = ActionOutput(JSONObject().apply { put("foo", "bar") })
        Assert.assertFalse(output.displayMessage(fakeCampaign))

        Mockito.verifyNoInteractions(actionModuleSpy)

        output = ActionOutput(JSONObject().apply { put("action", 2) })
        Assert.assertFalse(output.displayMessage(fakeCampaign))

        output = ActionOutput(JSONObject().apply { put("action", JSONObject()) })
        Assert.assertFalse(output.displayMessage(fakeCampaign))
