...
public class EventTriggerTest extends DITest
{
    ...
    public void setUp()
    {
        ...
        LocalCampaignsModule module = DITestUtils.mockSingletonDependency(LocalCampaignsModule.class,
                null);
    ...
    @Test
    public void testSignalSentAfterEventTracked() throws BatchMessagingException, InterruptedException, NoSuchFieldException, IllegalAccessException, JSONException
    {
        ...
        CampaignManagerProvider.get().updateCampaignList(Collections.singletonList(
                campaign));

        // Track the event which is linked to the Local Campaign
        Batch.User.trackEvent(EVENT_NAME_TEST);

        Mockito.verify(LocalCampaignsModuleProvider.get()).sendSignal(eventTrackedSignalEq(
                new EventTrackedSignal("E." + EVENT_NAME_TEST, null))
        );
    }

    @Test
    public void testCampaignDisplayedAfterEventTracked() throws BatchMessagingException, InterruptedException, NoSuchFieldException, IllegalAccessException, JSONException
    {
    ...
    static class EventTrackedSignalMatcher implements ArgumentMatcher<EventTrackedSignal>
    {
        ...
        {
            return expected == argument || expected != null && argument != null && expected.name.equals(
                    argument.name) && ( expected.parameters == null || expected.parameters.equals(
                    argument.parameters) );
        }
    }
