...
public class LocalCampaignsResponseSerializationTest {
  ...
  public void testValidSerialization() throws JSONException {
    ...
    Assert.assertEquals(
      response.getQueryID(),
      serializedResponse.getString("id")
    );
    ...
    JSONObject serializedCampaign = serializedResponse
      .getJSONArray("campaigns")
      .getJSONObject(0);
    Assert.assertEquals(
      campaign.id,
      serializedCampaign.getString("campaignId")
    );
    ...
    Assert.assertEquals(
      campaign.eventData,
      serializedCampaign.getJSONObject("eventData")
    );
    ...
    Assert.assertEquals(
      campaign.priority,
      serializedCampaign.getInt("priority")
    );
    Assert.assertEquals(
  ...
  public void testValidDeserialization() throws JSONException, IOException {
    ...
    Assert.assertEquals(
      validJsonCampaignsResponse.getString("id"),
      response.getQueryID()
    );
    ...
    JSONObject jsonCampaign = validJsonCampaignsResponse
      .getJSONArray("campaigns")
      .getJSONObject(0);
    LocalCampaign campaign = response.getCampaigns().get(0);
    ...
    Assert.assertNotNull(campaign.capping);
    Assert.assertEquals(
      Integer.valueOf(jsonCampaign.getInt("capping")),
      campaign.capping
    );
    Assert.assertEquals(
      jsonCampaign.getJSONObject("eventData"),
      campaign.eventData
    );
    Assert.assertNotNull(campaign.minimumAPILevel);
    Assert.assertEquals(
      Integer.valueOf(jsonCampaign.getInt("minimumApiLevel")),
      campaign.minimumAPILevel
    );
    Assert.assertNotNull(campaign.maximumAPILevel);
    Assert.assertEquals(
      Integer.valueOf(jsonCampaign.getInt("maximumApiLevel")),
      campaign.maximumAPILevel
    );
    Assert.assertEquals(jsonCampaign.getInt("priority"), campaign.priority);
    Assert.assertEquals(
      jsonCampaign.getInt("minDisplayInterval"),
      campaign.minimumDisplayInterval
    );
    Assert.assertNotNull(campaign.startDate);
    Assert.assertEquals(
      jsonCampaign.getJSONObject("startDate").getLong("ts"),
      campaign.startDate.getTime()
    );
    Assert.assertEquals(
    ...
    Assert.assertNotNull(campaign.endDate);
    Assert.assertEquals(
      jsonCampaign.getJSONObject("endDate").getLong("ts"),
      campaign.endDate.getTime()
    );
    Assert.assertEquals(
    ...
    Assert.assertTrue(campaign.output instanceof LandingOutput);
    Assert.assertEquals(
      jsonCampaign.getJSONObject("output").getJSONObject("payload"),
      campaign.output.payload
    );
  }
