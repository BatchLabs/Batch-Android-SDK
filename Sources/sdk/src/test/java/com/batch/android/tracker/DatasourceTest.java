...
public class DatasourceTest {
  ...
  public void testSimpleInsert() throws Exception {
    ...
    datasource.addEvent(
      new Event(appContext, new Date().getTime(), eventName, null)
    );
  ...
  public void testParamsInsert() throws Exception {
    ...
    datasource.addEvent(
      new Event(appContext, new Date().getTime(), eventName, parameters)
    );
  ...
  public void testMultipleInserts() throws Exception {
    ...
    datasource.addEvent(
      new Event(appContext, new Date().getTime(), normalEventName, null)
    );
    datasource.addEvent(
      new Event(appContext, new Date().getTime(), normalEventName, null)
    );
  ...
  public void testUpdateEvents() {
    ...
    datasource.updateEventsToOld(
      eventsIDs.toArray(new String[eventsIDs.size()])
    );
  ...
  private void insertBatchEvents(int number) {
    for (int i = 0; i < number; i++) {
      if (
        !datasource.addEvent(
          new Event(appContext, new Date().getTime(), "event" + i, null)
        )
      ) {
