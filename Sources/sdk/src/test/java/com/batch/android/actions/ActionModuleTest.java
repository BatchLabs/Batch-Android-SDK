...
public class ActionModuleTest extends DITest {
  ...
  public void testRemoveUserTagRegisterAction() throws JSONException {
    ...
    new ActionModule()
      .performAction(context, "batch.user.tag", jsonRemove, null);
  ...
  public void testGroupAction() throws JSONException {
    ...
    String groupJSON =
      "{'actions':[['first', {'foo': 'bar'}],[],['invalid'],['second']]}";

    actionModule.performAction(
      context,
      "batch.group",
      new JSONObject(groupJSON),
      null
    );
    ...
    actionModule.performAction(
      context,
      "batch.group",
      new JSONObject("{'actions':'bar'}"),
      null
    );
    actionModule.performAction(
      context,
      "batch.group",
      new JSONObject("{'actions':[]}"),
      null
    );
    actionModule.performAction(
      context,
      "batch.group",
      new JSONObject("{'actions':{'foo':'bar'}}"),
      null
    );
    actionModule.performAction(
      context,
  ...
  public void testGroupActionLimits() throws JSONException {
    ...
    String nestedAction =
      "{'actions':[['batch.group', {'actions': ['shouldNotRun']}]]}";
    actionModule.performAction(
      context,
      "batch.group",
      new JSONObject(nestedAction),
      null
    );
  ...
  public void testClipboardAction() throws JSONException {
    ...
    String clipboardJSON =
      "{'t':'best text ever', 'd':'best description ever'}";
    assertTrue(
      actionModule.performAction(
        context,
        "batch.clipboard",
        new JSONObject(clipboardJSON),
        null
      )
    );
