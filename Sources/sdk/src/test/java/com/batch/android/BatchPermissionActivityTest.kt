...
class BatchPermissionActivityTest {
    ...
    fun testActivityFinishedWhenPermissionAlreadyGranted() {
        val intent =
            Intent(ApplicationProvider.getApplicationContext(), BatchPermissionActivity::class.java)
        intent.putExtra(
            BatchPermissionActivity.EXTRA_PERMISSION,
            NotificationPermissionHelper.PERMISSION_NOTIFICATION,
        )
        scenario = ActivityScenario.launch(intent)
        scenario?.onActivity { Assert.assertTrue(it.isFinishing) }
    }

    /**
     * Test we get permission result from the broadcast receiver Also test the activity is finishing
     * well
     */
    @Test
    fun testActivityPermissionResult() {
        ...
        LocalBroadcastManagerProvider.get(ApplicationProvider.getApplicationContext())
            .registerReceiver(
                object : BroadcastReceiver() {
                    override fun onReceive(context: Context, intent: Intent) {
                        val permission =
                            intent.getStringExtra(BatchPermissionActivity.EXTRA_PERMISSION)
                        val granted =
                            intent.getBooleanExtra(BatchPermissionActivity.EXTRA_RESULT, false)
