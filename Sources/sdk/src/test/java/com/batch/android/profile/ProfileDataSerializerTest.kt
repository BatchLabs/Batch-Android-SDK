...
class ProfileDataSerializerTest {
    ...
    fun testSerialize() {
        ...
        val dataModel = ProfileUpdateOperation().apply {
            ...
            addAttribute("url_att", UserAttribute(URI("https://batch.com/pricing"), AttributeType.URL))
            addAttribute("date_att", UserAttribute(Date(1596975143943L), AttributeType.DATE))
            addAttribute("array_att", UserAttribute(listOf("michel", "bresil"), AttributeType.STRING_ARRAY))
        ...
        Assert.assertEquals("https://batch.com/pricing", actualCustomAttributes.getString("url_att.u"))
    ...
    fun testSerializePartialUpdateAttribute() {
        ...
        val expectedAddOnly = JSONObject().apply {
            put("\$add", JSONArray(listOf("a","b")))
        }
        Assert.assertNull(ProfileDataSerializer.serializePartialUpdateAttribute(attributeAddOnly).optJSONArray("\$remove"))
        Assert.assertEquals(expectedAddOnly.toString(), ProfileDataSerializer.serializePartialUpdateAttribute(attributeAddOnly).toString())

        // Case: Remove only
        val attributeRemoveOnly = ProfilePartialUpdateAttribute(null, listOf("a","b"))
        val expectedRemoveOnly = JSONObject().apply {
            put("\$remove", JSONArray(listOf("a","b")))
        }
        Assert.assertNull(ProfileDataSerializer.serializePartialUpdateAttribute(attributeRemoveOnly).optJSONArray("\$add"))
        Assert.assertEquals(expectedRemoveOnly.toString(), ProfileDataSerializer.serializePartialUpdateAttribute(attributeRemoveOnly).toString())

        // Case: Both
        val attributeBoth = ProfilePartialUpdateAttribute(listOf("a","b"), listOf("c","d"))
        val expectedBoth = JSONObject().apply {
            put("\$add", JSONArray(listOf("a","b")))
            put("\$remove", JSONArray(listOf("c","d")))
        }
        Assert.assertNotNull(ProfileDataSerializer.serializePartialUpdateAttribute(attributeBoth).optJSONArray("\$add"))
        Assert.assertNotNull(ProfileDataSerializer.serializePartialUpdateAttribute(attributeBoth).optJSONArray("\$remove"))
        Assert.assertEquals(expectedBoth.toString(), ProfileDataSerializer.serializePartialUpdateAttribute(attributeBoth).toString())
