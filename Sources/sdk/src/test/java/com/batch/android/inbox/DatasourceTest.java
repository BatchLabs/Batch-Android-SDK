...
public class DatasourceTest {
  ...
  public void testInsertFetcherIds() {
    assertEquals(
      -1,
      datasource.getFetcherID(FetcherType.USER_IDENTIFIER, null)
    );
    assertEquals(-1, datasource.getFetcherID(FetcherType.USER_IDENTIFIER, ""));

    long fetcherId = datasource.getFetcherID(
      FetcherType.USER_IDENTIFIER,
      "test-custom-id"
    );
    ...
    assertEquals(
      1,
      cursor.getInt(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_FETCHER_TYPE)
      )
    );
    assertEquals(
      "test-custom-id",
      cursor.getString(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_FETCHER_IDENTIFIER)
      )
    );

    long fetcherId2 = datasource.getFetcherID(
      FetcherType.INSTALLATION,
    ...
    assertEquals(
      0,
      cursor2.getInt(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_FETCHER_TYPE)
      )
    );
    assertEquals(
      "b5baf3e0-a01f-11ea-111a-17c13e111be2",
      cursor2.getString(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_FETCHER_IDENTIFIER)
      )
    );

    long fetcherId3 = datasource.getFetcherID(
      FetcherType.USER_IDENTIFIER,
      "test-custom-id"
    );
  ...
  public void testGetNotifications() {
    ...
    NotificationIdentifiers identifiers2 = new NotificationIdentifiers(
      "test-id-2",
      "test-send-id-2"
    );
  ...
  public void testSimpleInsert() {
    ...
    assertEquals(
      "test-id",
      cursor.getString(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_NOTIFICATION_ID)
      )
    );
    assertEquals(
      "test-send-id",
      cursor.getString(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_SEND_ID)
      )
    );
    assertEquals(
      "test title",
    ...
    assertEquals(
      ...
      cursor.getString(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_PAYLOAD)
      )
    );
    ...
    assertEquals(
      fetcherId,
      cursor2.getLong(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_FETCHER_ID)
      )
    );
    assertEquals(
      "test-id",
      cursor2.getString(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_NOTIFICATION_ID)
      )
    );
    assertEquals(
      "b5baf3e0-a01f-11ea-111a-17c13e111be2",
      cursor2.getString(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_INSTALL_ID)
      )
    );
    assertNull(
      cursor2.getString(
        cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_CUSTOM_ID)
      )
    );
  }
  ...
  public void testUpdate() throws JSONException {
    ...
    assertEquals(
      "test-id",
      datasource.updateNotification(updatePayload, fetcherId)
    );

    updatePayload =
      new JSONObject("{\"notificationId\":\"test-id\",\"read\":true}");
    assertEquals(
      "test-id",
      datasource.updateNotification(updatePayload, fetcherId)
    );
  ...
  public void testMarkNotificationAsDeleted() {
    ...
    boolean deleted =
      cursor.getInt(
        cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_DELETED)
      ) !=
      0;
    assertFalse(deleted);
  ...
  public void testMarkAllAsRead() {
    long fetcherIdInstall = datasource.getFetcherID(
      FetcherType.INSTALLATION,
      "test-install-id"
    );
  ...
  public void testDelete() {
    ...
    assertTrue(datasource.insert(notification, fetcherId));
    assertTrue(
      datasource.deleteNotifications(Collections.singletonList("test-id"))
    );
  ...
  public void testGetCandidate() {
    ...
    candidates =
      datasource.getCandidateNotifications("test-id-1", 2, fetcherId);
    assertEquals(2, candidates.size());
    assertEquals("test-id-2", candidates.get(0).identifier);
    assertEquals("test-id-3", candidates.get(1).identifier);

    candidates =
      datasource.getCandidateNotifications("test-id-3", 2, fetcherId);
  ...
  public void testGetCandidateMultipleFetcher() {
    long fetcherIdCustom = datasource.getFetcherID(
      FetcherType.USER_IDENTIFIER,
      "test-custom-id"
    );
    ...
    candidates =
      datasource.getCandidateNotifications("test-id-1", 10, fetcherIdInstall);
    assertEquals(2, candidates.size());
    assertEquals("test-id-2", candidates.get(0).identifier);
    assertEquals("test-id-3", candidates.get(1).identifier);

    candidates =
      datasource.getCandidateNotifications("test-id-3", 10, fetcherIdInstall);
    assertEquals(0, candidates.size());
