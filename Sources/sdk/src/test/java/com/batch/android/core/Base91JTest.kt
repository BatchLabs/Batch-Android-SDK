...
class Base91JTest {
    ...
    private val b91 = Base91J()

    private val simpleCase = "ceciestuntest"
    private val simpleCaseEncoded = "OPWfd,DZ7U)/Tm{o"

    private val simpleCase4 = "ceciestuntest❤"
    private val simpleCaseEncoded4 = "OPWfd,DZ7U)/Tm{oG_qO"

    private val simpleCase3 = "ceciestuntest❤️"
    private val simpleCaseEncoded3 = "OPWfd,DZ7U)/Tm{oG_~[DoOD"
    ...
    private val complexJSONEncoded =
    ...
    @Test
    fun testEncode() {
        Assert.assertEquals(simpleCaseEncoded, b91.encodeString(simpleCase))
        Assert.assertEquals(simpleCaseEncoded3, b91.encodeString(simpleCase3))
        Assert.assertEquals(simpleCaseEncoded4, b91.encodeString(simpleCase4))
        Assert.assertEquals(complexJSONEncoded, b91.encodeString(complexJSON))
    }

    @Test
    fun testDecode() {
        Assert.assertEquals(simpleCase, b91.decodeString(simpleCaseEncoded).decodeToString())
        Assert.assertEquals(simpleCase3, b91.decodeString(simpleCaseEncoded3).decodeToString())
        Assert.assertEquals(simpleCase4, b91.decodeString(simpleCaseEncoded4).decodeToString())
        Assert.assertEquals(complexJSON, b91.decodeString(complexJSONEncoded).decodeToString())
    }
}
