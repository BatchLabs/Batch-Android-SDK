...
{
    private String payloadJson = "{\"t\":\"c\",\"l\":\"https://batch.com\",\"i\":\"test-send-id\",\"od\":{\"n\":\"02521fa3e-70e4-11ea-b5ac-bf057dd56464\",\"ct\":\"2918f6a1d46ff48c9e74bfded5d5d9c9e\"}}";
    ...
    public void testMissingNotificationInCache() throws Exception
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            NotificationIdentifiers identifiers = new NotificationIdentifiers("test-id-" + i,
                    "test-send-id-" + i);
        ...
        InboxSyncWebserviceClient client = PowerMockito.spy(new InboxSyncWebserviceClient(
                appContext,
                USER_IDENTIFIER,
                "test-id",
                "test-auth",
                ...
                new InboxWebserviceListener()
                {
                    ...
                    public void onSuccess(InboxWebserviceResponse result)
                    {
                        ...
                        assertEquals("test-id-2.5",
                                result.notifications.get(0).identifiers.identifier);
                        assertEquals("test-id-2",
                                result.notifications.get(1).identifiers.identifier);
                        ...
                        assertEquals("b5baf3e0-a01f-11ea-111a-17c13e111be2",
    ...
    public void testUpdatingNotificationInCache() throws Exception
    {
                ...
                new InboxWebserviceListener()
                {
                    ...
                    public void onSuccess(InboxWebserviceResponse result)
                    {
                        ...
                        assertEquals("test-id-1",
                                result.notifications.get(1).identifiers.identifier);
                        assertEquals("test-install-id-1-updated",
                                result.notifications.get(1).identifiers.installID);
                        assertEquals("test-custom-id-1-updated",
                                result.notifications.get(1).identifiers.customID);
                        assertEquals("test-send-id-1-updated",
                                result.notifications.get(1).identifiers.sendID);
                        assertEquals(7300000,
                                result.notifications.get(1).date.getTime());
                        ...
                        assertEquals("test-id-1",
                                cursor2.getString(cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_NOTIFICATION_ID)));
                        assertEquals(1,
                                cursor2.getInt(cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_UNREAD)));
                        assertEquals(7300000,
                                cursor2.getInt(cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_DATE)));
                    }
        ...
        String fakeServerResponse = "{\"notifications\":[{\"read\":true,\"notificationId\":\"test-id-2\"},{\"installId\":\"test-install-id-1-updated\",\"customId\":\"test-custom-id-1-updated\",\"notificationId\":\"test-id-1\",\"notificationTime\":7300000,\"sendId\":\"test-send-id-1-updated\"}],\"hasMore\":true,\"timeout\":false,\"cursor\":\"test-id-1\"}";
        PowerMockito.doReturn(new JSONObject(fakeServerResponse)).when(client,
                "getBasicJsonResponseBody");
        client.run();
    }
