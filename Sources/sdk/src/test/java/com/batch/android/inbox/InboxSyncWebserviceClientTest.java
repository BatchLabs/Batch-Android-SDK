...
public class InboxSyncWebserviceClientTest {
  ...
  public void testMissingNotificationInCache() throws Exception {
        ...
        new InboxWebserviceListener() {
          ...
          public void onSuccess(InboxWebserviceResponse result) {
            ...
            assertEquals(
              "test title",
              cursor.getString(
                cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_TITLE)
              )
            );
            assertEquals(
              "test body",
              cursor.getString(
                cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_BODY)
              )
            );
            assertEquals(
              1,
              cursor.getInt(
                cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_UNREAD)
              )
            );
            assertEquals(
              10900000,
              cursor.getLong(
                cursor.getColumnIndex(InboxDatabaseHelper.COLUMN_DATE)
              )
            );
    ...
    PowerMockito
      .doReturn(new JSONObject(fakeServerResponse))
      .when(client, "getBasicJsonResponseBody");
    client.run();
  }
      ...
      new InboxSyncWebserviceClient(
        ...
        new InboxWebserviceListener() {
          ...
          public void onSuccess(InboxWebserviceResponse result) {
            ...
            assertEquals(
              "test-install-id-1-updated",
              result.notifications.get(1).identifiers.installID
            );
            assertEquals(
              "test-custom-id-1-updated",
            ...
            assertEquals(
              1,
              cursor2.getInt(
                cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_UNREAD)
              )
            );
            assertEquals(
              7300000,
              cursor2.getInt(
                cursor2.getColumnIndex(InboxDatabaseHelper.COLUMN_DATE)
              )
            );
          }
