...
public class RatingActionRunnable implements UserActionRunnable {
  ...
  public static final String IDENTIFIER =
    ActionModule.RESERVED_ACTION_IDENTIFIER_PREFIX + "rating";
  ...
  public void performAction(
    ...
    if (context == null) {
      Logger.error(
        TAG,
        "Tried to perform a Rating action, but no context was available"
      );
      return;
    }
    tryOpenPlayStoreInAppRating(context)
      .catchException(e -> {
        String message = (e instanceof RatingActionRunnableException)
          ? e.getMessage()
          : null;
        if (message == null) {
          message =
            "This device might not have Google's services, or the Play Store version is too old.";
        }
        Logger.error(
          ActionModule.TAG,
    ...
    return new Promise<>(promise -> {
      try {
        ...
        if (supportActivity == null) {
          throw new RatingActionRunnableException(
            "Could not find an Activity to use for display.",
            null
          );
        }
        ...
        reviewManager
          ...
          .addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
              ...
              if (reviewInfo == null) {
                promise.reject(
                  new RatingActionRunnableException(
                    "Invalid reply from Play Store (-10).",
                    null
                  )
                );
              }
        ...
        throw new RatingActionRunnableException(
          "Is 'com.google.android.play:core' available?",
          null
        );
      }
    });
  ...
  private void openStore(@NonNull Context context) {
    Logger.info(
      ActionModule.TAG,
      "In-app rating unavailable: trying to open store application."
    );
