...
public class FlexboxLayout extends ViewGroup {
  ...
  private void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    // loop)
    {
      ...
      for (int i = 0; i < childCount; i++) {
        ...
        if (
          lp.flexBasisPercent != LayoutParams.FLEX_BASIS_PERCENT_DEFAULT &&
        ...
        childState =
          ViewCompat.combineMeasuredStates(
            childState,
        ...
        if (mFlexWrap != FLEX_WRAP_WRAP_REVERSE) {
          flexLine.maxBaseline =
            Math.max(flexLine.maxBaseline, child.getBaseline() + lp.topMargin);
        } else {
  ...
  private void addFlexLineIfLastFlexItem(
    int childIndex,
    int childCount,
    FlexLine flexLine
  ) {
   ...
   */
  private void determineMainSize(
  ...
  private int expandFlexItems(
    ...
    float unitSpace =
      (maxMainSize - flexLine.mainSize) / flexLine.totalFlexGrow;
      ...
      if (
        ...
        if (!mChildrenFrozen[childIndex]) {
          float rawCalculatedWidth =
            child.getMeasuredWidth() + unitSpace * lp.flexGrow;
        ...
        if (!mChildrenFrozen[childIndex]) {
          float rawCalculatedHeight =
            child.getMeasuredHeight() + unitSpace * lp.flexGrow;
          if (i == flexLine.itemCount - 1) {
  ...
  private int shrinkFlexItems(
    ...
    float unitShrink =
      (flexLine.mainSize - maxMainSize) / flexLine.totalFlexShrink;
    float accumulatedRoundError = 0;
    flexLine.mainSize = paddingAlongMainAxis;
    for (int i = 0; i < flexLine.itemCount; i++) {
      View child = getReorderedChildAt(childIndex);
      ...
      if (
        ...
        if (!mChildrenFrozen[childIndex]) {
          float rawCalculatedWidth =
            child.getMeasuredWidth() - unitShrink * lp.flexShrink;
          if (i == flexLine.itemCount - 1) {
            rawCalculatedWidth += accumulatedRoundError;
            accumulatedRoundError = 0;
          }
        ...
        if (!mChildrenFrozen[childIndex]) {
          float rawCalculatedHeight =
            child.getMeasuredHeight() - unitShrink * lp.flexShrink;
          if (i == flexLine.itemCount - 1) {
            rawCalculatedHeight += accumulatedRoundError;
            accumulatedRoundError = 0;
          }
   ...
   * @see #setAlignContent(int)
   */
  private void determineCrossSize(
    int flexDirection,
    int widthMeasureSpec,
    int heightMeasureSpec,
      ...
      if (mFlexLines.size() == 1) {
        ...
        switch (mAlignContent) {
          case ALIGN_CONTENT_STRETCH:
            {
              float freeSpaceUnit =
                (size - totalCrossSize) / (float) mFlexLines.size();
                ...
                if (i != mFlexLines.size() - 1) {
                  ...
                  if (i == mFlexLines.size() - 2) {
                    ...
                    dummySpaceFlexLine.crossSize =
                      Math.round(spaceBetweenFlexLine + accumulatedError);
                    accumulatedError = 0;
                  } else {
                    dummySpaceFlexLine.crossSize =
                      Math.round(spaceBetweenFlexLine);
                  }
                  accumulatedError +=
                    (spaceBetweenFlexLine - dummySpaceFlexLine.crossSize);
                  if (accumulatedError > 1) {
    ...
    if (alignItems == ALIGN_ITEMS_STRETCH) {
      ...
      for (FlexLine flexLine : mFlexLines) {
        for (int i = 0; i < flexLine.itemCount; i++, viewIndex++) {
          ...
          LayoutParams lp = (LayoutParams) view.getLayoutParams();
          if (
            lp.alignSelf != LayoutParams.ALIGN_SELF_AUTO &&
  ...
  private void setMeasuredDimensionForFlex(
    ...
    switch (flexDirection) {
      ...
      case FLEX_DIRECTION_ROW_REVERSE:
        calculatedMaxHeight =
          getSumOfCrossSize() + getPaddingTop() + getPaddingBottom();
      ...
      case FLEX_DIRECTION_COLUMN_REVERSE:
        ...
        calculatedMaxWidth =
          getSumOfCrossSize() + getPaddingLeft() + getPaddingRight();
        break;
      default:
        throw new IllegalArgumentException(
          "Invalid flex direction: " + flexDirection
    ...
    switch (widthMode) {
      ...
      default:
        throw new IllegalStateException(
          "Unknown width mode is set: " + widthMode
        );
    }
    ...
    switch (heightMode) {
      case MeasureSpec.EXACTLY:
        if (heightSize < calculatedMaxHeight) {
          childState =
            ViewCompat.combineMeasuredStates(
              childState,
              ViewCompat.MEASURED_STATE_TOO_SMALL >> ViewCompat.MEASURED_HEIGHT_STATE_SHIFT
            );
        }
        heightSizeAndState =
          ViewCompat.resolveSizeAndState(
      ...
      default:
        throw new IllegalStateException(
          "Unknown height mode is set: " + heightMode
        );
    }
  ...
  protected void onLayout(
    ...
    switch (mFlexDirection) {
      ...
      default:
        throw new IllegalStateException(
          "Invalid flex direction is set: " + mFlexDirection
        );
    }
  }
  ...
  private void layoutHorizontal(
    ...
    for (FlexLine flexLine : mFlexLines) {
      ...
      switch (mJustifyContent) {
        ...
        case JUSTIFY_CONTENT_SPACE_AROUND:
          if (flexLine.itemCount != 0) {
            spaceBetweenItem =
              (width - flexLine.mainSize) / (float) flexLine.itemCount;
          }
        ...
        case JUSTIFY_CONTENT_SPACE_BETWEEN:
          childLeft = paddingLeft;
          float denominator = flexLine.itemCount != 1
            ? flexLine.itemCount - 1
            : 1f;
      ...
      for (int i = 0; i < flexLine.itemCount; i++) {
        ...
        childLeft +=
          child.getMeasuredWidth() + spaceBetweenItem + lp.rightMargin;
        childRight -=
          child.getMeasuredWidth() + spaceBetweenItem + lp.leftMargin;
        currentViewIndex++;
      }
   ...
   * @see LayoutParams#alignSelf
   */
  private void layoutSingleChildHorizontal(
    View view,
    FlexLine flexLine,
    ...
    switch (alignItems) {
      ...
      case ALIGN_ITEMS_FLEX_END:
        if (flexWrap != FLEX_WRAP_WRAP_REVERSE) {
          ...
          view.layout(
            left,
            top - crossSize + view.getMeasuredHeight() + lp.topMargin,
            right,
      ...
      case ALIGN_ITEMS_CENTER:
        ...
        if (flexWrap != FLEX_WRAP_WRAP_REVERSE) {
          view.layout(
            left,
            top + topFromCrossAxis + lp.topMargin - lp.bottomMargin,
            right,
            top +
            topFromCrossAxis +
            view.getMeasuredHeight() +
    ...
    for (FlexLine flexLine : mFlexLines) {
      ...
      switch (mJustifyContent) {
        ...
        case JUSTIFY_CONTENT_CENTER:
          ...
          childBottom =
            height - paddingBottom - (height - flexLine.mainSize) / 2f;
          break;
        case JUSTIFY_CONTENT_SPACE_AROUND:
          if (flexLine.itemCount != 0) {
            spaceBetweenItem =
              (height - flexLine.mainSize) / (float) flexLine.itemCount;
          }
      ...
      for (int i = 0; i < flexLine.itemCount; i++) {
        ...
        childTop +=
          child.getMeasuredHeight() + spaceBetweenItem + lp.bottomMargin;
        childBottom -=
          child.getMeasuredHeight() + spaceBetweenItem + lp.topMargin;
        currentViewIndex++;
      }
   ...
   * @see LayoutParams#alignSelf
   */
  private void layoutSingleChildVertical(
    View view,
    FlexLine flexLine,
    boolean isRtl,
    int alignItems,
    ...
    switch (alignItems) {
      ...
      case ALIGN_ITEMS_FLEX_END:
        if (!isRtl) {
          view.layout(
            left + crossSize - view.getMeasuredWidth() - lp.rightMargin,
            top,
            right + crossSize - view.getMeasuredWidth() - lp.rightMargin,
            bottom
          );
        } else {
          // If the flexWrap == FLEX_WRAP_WRAP_REVERSE, the direction of the
          ...
          view.layout(
            left - crossSize + view.getMeasuredWidth() + lp.leftMargin,
            top,
      ...
      case ALIGN_ITEMS_CENTER:
        ...
        if (!isRtl) {
          view.layout(
            left + leftFromCrossAxis + lp.leftMargin - lp.rightMargin,
            top,
            right + leftFromCrossAxis + lp.leftMargin - lp.rightMargin,
            bottom
          );
        } else {
          view.layout(
  ...
  public static class LayoutParams extends MarginLayoutParams {
    ...
    private static final int MAX_SIZE =
      Integer.MAX_VALUE & ViewCompat.MEASURED_SIZE_MASK;
