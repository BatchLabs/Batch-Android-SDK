...
public class ModalTemplateFragment extends BaseDialogFragment<ModalMessage> implements BannerView.OnActionListener, ModalContentPanGestureDetector.OnDismissListener
{
    ...
    public static ModalTemplateFragment newInstance(BatchMessage payloadMessage,
                                                    ModalMessage messageModel)
    {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState)
    {
        ...
        setStyle(STYLE_NO_FRAME,
                showStatusbar ? R.style.com_batchsdk_ModalDialogTheme
                        : R.style.com_batchsdk_ModalDialogTheme_Fullscreen);
    ...
    private View getBannerView(Context context)
    {
        ...
        final Context c = new ContextThemeWrapper(context,
                ThemeHelper.getDefaultLightTheme(context));
        final BannerView bannerView = new BannerView(c,
                model,
                getStyle(),
                new DOMNode("container"),
                this);
        bannerView.setActionListener(this);

        if (model.allowSwipeToDismiss) {
            ModalContentPanGestureDetector detector = new ModalContentPanGestureDetector(context,
                    false);
        ...
        StyleHelper.applyCommonRules(view,
    ...
    public void onGlobalAction()
    {
        if (!dismissed) {
            ...
            if (message.globalTapAction != null) {
                messagingModule.performAction(getContext(),
                        getPayloadMessage(),
