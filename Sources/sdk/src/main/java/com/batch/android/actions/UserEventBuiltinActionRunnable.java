...
public class UserEventBuiltinActionRunnable implements UserActionRunnable {
  ...
  public static String IDENTIFIER =
    ActionModule.RESERVED_ACTION_IDENTIFIER_PREFIX + "user.event";
  ...
  public void performAction(
    ...
    try {
      ...
      if (event.length() == 0) {
        Logger.internal(
          TAG,
          "Could not perform event action : event name is empty"
        );
        return;
      }
      ...
      if (tags != null && tags.length() > 0) {
        for (int i = 0; i < tags.length(); i++) {
          ...
          if (tag != null && tag.length() > 0) {
            ...
            Logger.internal(
              TAG,
              "Could not add tag in event action : tag value is null or invalid"
            );
          }
        }
      }
      ...
      if (argsData != null && argsData.keySet().size() > 0) {
        for (String key : argsData.keySet()) {
          ...
          if (toAdd instanceof Integer) {
            ...
            Logger.internal(
              TAG,
              "Could not add data in event action : value type is invalid"
            );
          }
        }
      }
