...
public class LocalCampaignsFilePersistence
  implements LocalCampaignsPersistence {
  ...
  public void persistData(
    ...
    try {
      ...
      if (!tmpFile.createNewFile()) {
        Logger.internal(
          TAG,
          "Unable to create a temporary file " + file.getName()
        );
      }
      ...
      if (!outputJSON.has(PERSISTENCE_SAVE_VERSION_KEY)) {
        outputJSON.put(
          PERSISTENCE_SAVE_VERSION_KEY,
          PERSISTENCE_CURRENT_FILE_VERSION
        );
      }
      ...
      if (file.exists()) {
        if (!file.delete()) {
          Logger.internal(
            TAG,
            "Unable to create previous cached file " + file.getName()
          );
        }
      }
      ...
      if (tmpFile.exists()) {
        if (!tmpFile.delete()) {
          throw new PersistenceException(
            "unable to delete temporary cached file " + tmpFile.getName(),
            ex
          );
        }
      }
      ...
      throw new PersistenceException(
        "Can't save json file. " + ex.toString(),
        ex
      );
    } finally {
      try {
  ...
  public JSONObject loadData(
    ...
    try {
      ...
      if (json.has(PERSISTENCE_SAVE_VERSION_KEY)) {
        if (
          json.getInt(PERSISTENCE_SAVE_VERSION_KEY) ==
          PERSISTENCE_CURRENT_FILE_VERSION
        ) {
          return json;
        } else {
      ...
      throw new PersistenceException(
        "Can't parse loaded json response. " + ex.toString(),
        ex
      );
    }
  ...
  @Override
  public void deleteData(@NonNull Context context, @NonNull String filename)
    throws PersistenceException {
    final File cacheDir = context.getCacheDir();
