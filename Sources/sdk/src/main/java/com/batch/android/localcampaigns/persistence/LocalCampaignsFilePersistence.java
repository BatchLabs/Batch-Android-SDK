...
public class LocalCampaignsFilePersistence implements LocalCampaignsPersistence
{
    ...
    @Override
    public boolean hasSavedData(@NonNull Context context,
                                @NonNull String filename) throws PersistenceException
    {
        return new File(context.getCacheDir(), filename).exists();
    }

    @Override
    public void persistData(@NonNull Context context,
                            ...
                            @NonNull String filename) throws PersistenceException
    {
        ...
        File tmpFile = new File(cacheDir,
                PERSISTENCE_TMP_FILE_PREFIX + UUID.randomUUID().toString() + ".json");
        ...
        try {
            ...
            if (tmpFile.exists()) {
                if (!tmpFile.renameTo(file)) {
                    String removedMsg = tmpFile.delete() ? "Removed temporary cache file" : "Tried to remove temporary cache file but unable to do it.";

                    throw new PersistenceException("unable to rename temporary cached file " + tmpFile.getName() + " to " + file.getName() +
                            ". " + removedMsg);
                }
            } else {
            ...
            if (tmpFile.exists()) {
                if (!tmpFile.delete()) {
                    throw new PersistenceException("unable to delete temporary cached file " + tmpFile.getName(),
                            ex);
                }
            }
    ...
    @Nullable
    @Override
    public JSONObject loadData(@NonNull Context context,
                               @NonNull String filename) throws PersistenceException
    {
        FileInputStream fis;
        ...
        try {
            ...
            if (json.has(PERSISTENCE_SAVE_VERSION_KEY)) {
                if (json.getInt(PERSISTENCE_SAVE_VERSION_KEY) == PERSISTENCE_CURRENT_FILE_VERSION) {
                    ...
                    throw new PersistenceException(
                            "The loaded file has a wrong version. Dropping.");
                }
            }
        } catch (JSONException ex) {
            throw new PersistenceException("Can't parse loaded json response. " + ex.toString(),
                    ex);
        }
    ...
    @Override
    public void deleteData(@NonNull Context context,
                           @NonNull String filename) throws PersistenceException
    {
        final File cacheDir = context.getCacheDir();
