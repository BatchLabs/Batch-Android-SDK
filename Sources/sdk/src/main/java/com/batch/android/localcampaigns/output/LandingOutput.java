...
public class LandingOutput extends LocalCampaign.Output
{
    ...
    public boolean displayMessage(LocalCampaign campaign)
    {
        try {
            ...
            JSONObject customPayload = new JSONObject(campaign.customPayload != null ? new JSONObject(
                    campaign.customPayload) : new JSONObject());

            BatchInAppMessage message = new BatchInAppMessage(campaign.publicToken,
                    campaign.id,
                    campaign.eventData,
                    mergedPayload,
                    customPayload);
