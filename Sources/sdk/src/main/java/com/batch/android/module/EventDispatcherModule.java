...
public class EventDispatcherModule extends BatchModule {
  ...
  private static final String COMPONENT_KEY_PREFIX =
    "com.batch.android.eventdispatcher:";
  ...
  public void dispatchEvent(
    Batch.EventDispatcher.Type type,
    Batch.EventDispatcher.Payload params
  ) {
  ...
  public void loadDispatcherFromContext(Context context) {
    if (Boolean.TRUE.equals(optOutModule.isOptedOutSync(context))) {
      Logger.internal(
        TAG,
        "Batch is opted out, refusing to add event dispatchers."
      );
      return;
    }
