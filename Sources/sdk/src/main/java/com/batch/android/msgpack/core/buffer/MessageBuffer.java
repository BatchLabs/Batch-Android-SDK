...
public class MessageBuffer {
  ...
  private static final String UNIVERSAL_MESSAGE_BUFFER =
    "com.batch.android.msgpack.core.buffer.MessageBufferU";
  private static final String BIGENDIAN_MESSAGE_BUFFER =
    "com.batch.android.msgpack.core.buffer.MessageBufferBE";
  private static final String DEFAULT_MESSAGE_BUFFER =
    ...
    try {
      ...
      useUniversalBuffer =
        Boolean.parseBoolean(
          System.getProperty("msgpack.universal-buffer", "false")
        ) ||
        isAndroid ||
        isGAE ||
        javaVersion < 7 ||
      ...
      if (isUniversalBuffer) {
        ...
        bufferClsName =
          isLittleEndian ? DEFAULT_MESSAGE_BUFFER : BIGENDIAN_MESSAGE_BUFFER;
      }
      ...
      if (DEFAULT_MESSAGE_BUFFER.equals(bufferClsName)) {
        ...
        try {
          ...
          Constructor<?> mbArrCstr = bufferCls.getDeclaredConstructor(
            byte[].class,
            int.class,
            int.class
          );
          ...
          Constructor<?> mbBBCstr = bufferCls.getDeclaredConstructor(
            ByteBuffer.class
          );
   ...
   */
  private static MessageBuffer newInstance(
  ...
  public static void releaseBuffer(MessageBuffer buffer) {
    if (isUniversalBuffer || buffer.hasArray()) {
      ...
      DirectBufferAccess.isDirectByteBufferInstance(buffer.reference)
    ) {
  ...
  MessageBuffer(ByteBuffer bb) {
    if (bb.isDirect()) {
      ...
      throw new IllegalArgumentException(
        "Only the array-backed ByteBuffer or DirectBuffer is supported"
      );
    }
  }
  ...
  public void putByteBuffer(int index, ByteBuffer src, int len) {
    ...
    if (src.isDirect()) {
      unsafe.copyMemory(
        null,
        DirectBufferAccess.getAddress(src) + src.position(),
        base,
        address + index,
        len
      );
      ...
      unsafe.copyMemory(
        srcArray,
        ARRAY_BYTE_BASE_OFFSET + src.position(),
        base,
        address + index,
        len
      );
  ...
  public ByteBuffer sliceAsByteBuffer(int index, int length) {
    if (hasArray()) {
      return ByteBuffer.wrap(
        (byte[]) base,
        (int) ((address - ARRAY_BYTE_BASE_OFFSET) + index),
        length
      );
    } else {
      if (isUniversalBuffer) {
