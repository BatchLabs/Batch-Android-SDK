...
final class OperatorProvider {
    ...
    static Value performNumberOperationOnValues(
        ArrayList<PrimitiveValue> values,
        String symbol,
        NumberOperation operation
    ) {
        if (values.size() < 2) {
            return new ErrorValue(ErrorValue.Type.Error, symbol + ": requires at least two arguments");
        }
        ...
        for (int i = 1; i < values.size(); i++) {
            ...
            if (atomValue == null) {
                return new ErrorValue(ErrorValue.Type.Internal, "=: value can't be nil. Are we out of bounds?");
            }
    ...
    static Value performSetOperationOnValues(ArrayList<PrimitiveValue> values, String symbol, SetOperation operation) {
        if (values.size() != 2) {
        ...
        if (
            wantedSetPrimitive.type != PrimitiveValue.Type.StringSet ||
            targetSetPrimitive.type != PrimitiveValue.Type.StringSet
        ) {
            return new ErrorValue(ErrorValue.Type.Error, symbol + ": all arguments should be string sets");
        }
     ...
     */
    static Value performStringOperationOnValues(
        ArrayList<PrimitiveValue> values,
        String symbol,
        StringOperation operation
    ) {
        if (values.size() != 1) {
            return new ErrorValue(ErrorValue.Type.Error, symbol + ": requires only one string/set argument");
        }
        ...
        return new ErrorValue(ErrorValue.Type.Error, symbol + ": argument should be a string or a set");
    }
}
...
class IfOperatorHandler implements OperatorHandler {
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        ...
        if (nbArgs < 2 || nbArgs > 3) {
            return new ErrorValue(ErrorValue.Type.Error, "if: should be called with 2 or 3 arguments");
        }
        ...
        if (condition.type == PrimitiveValue.Type.Nil) {
            ...
            return new ErrorValue(ErrorValue.Type.Error, "if: condition should be nil or a boolean value");
        }
    }
}
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        ...
        for (PrimitiveValue atomValue : values) {
            ...
            if (atomValue.type != PrimitiveValue.Type.Bool || !(atomValue.value instanceof Boolean)) {
                return new ErrorValue(ErrorValue.Type.Error, "and: Cannot compare non boolean values");
            }
...
class OrOperatorHandler implements OperatorHandler {
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        ...
        for (PrimitiveValue atomValue : values) {
            ...
            if (atomValue.type != PrimitiveValue.Type.Bool || !(atomValue.value instanceof Boolean)) {
                return new ErrorValue(ErrorValue.Type.Error, "or: Cannot compare non boolean values");
            }
...
class GreaterThanOperatorHandler implements OperatorHandler {
    ...
    @Override
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        return OperatorProvider.performNumberOperationOnValues(
            values,
            ">",
...
class GreaterThanOrEqualOperatorHandler implements OperatorHandler {
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        return OperatorProvider.performNumberOperationOnValues(
            ...
            ">=",
            (referenceValue, currentValue) ->
                referenceValue.doubleValue() > currentValue.doubleValue() || referenceValue.equals(currentValue)
        );
    }
}
...
class ContainsOperatorHandler implements OperatorHandler {
    ...
    @Override
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        return OperatorProvider.performSetOperationOnValues(
            values,
            "contains",
            (source, target) -> {
...
class LowerOperatorHandler implements OperatorHandler {
    ...
    @Override
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        return OperatorProvider.performStringOperationOnValues(
            values,
            "lower",
...
class ParseStringOperatorHandler implements OperatorHandler {
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        if (values.size() != 1) {
            return new ErrorValue(ErrorValue.Type.Error, "parse-string: only accepts one string argument");
        }
        ...
        if (value.type != PrimitiveValue.Type.String) {
            return new ErrorValue(ErrorValue.Type.Error, "parse-string: only accepts a string argument");
        }
...
class WriteToStringOperatorHandler implements OperatorHandler {
    ...
    public Value run(EvaluationContext context, ArrayList<PrimitiveValue> values) {
        if (values.size() != 1) {
            return new ErrorValue(ErrorValue.Type.Error, "write-to-string: only accepts a single, non-set argument");
        }
