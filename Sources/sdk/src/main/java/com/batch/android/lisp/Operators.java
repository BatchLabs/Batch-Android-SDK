...
final class OperatorProvider {
  ...
  private void setupOperators() {
    ...
    addOperator(
      new Operator("write-to-string", new WriteToStringOperatorHandler())
    );
  }
  ...
  static Value performNumberOperationOnValues(
    ...
    for (int i = 1; i < values.size(); i++) {
      ...
      if (
        !operation.performOperation(referenceNumber, (Number) atomValue.value)
      ) {
  ...
  static Value performSetOperationOnValues(
    ArrayList<PrimitiveValue> values,
    String symbol,
    ...
    if (values.size() != 2) {
      return new ErrorValue(
        ErrorValue.Type.Error,
        symbol + ": only accepts two arguments"
      );
    }
    ...
    if (
      !(wantedSetPrimitive.value instanceof Set) ||
      !(targetSetPrimitive.value instanceof Set)
    ) {
      return new ErrorValue(
        ErrorValue.Type.Error,
        symbol +
        ": internal consistency error: all arguments should be of underlying type Set"
      );
    }
...
class AndOperatorHandler implements OperatorHandler {
  ...
  public Value run(
    ...
    for (PrimitiveValue atomValue : values) {
      ...
      if (
        atomValue.type != PrimitiveValue.Type.Bool ||
        !(atomValue.value instanceof Boolean)
      ) {
        return new ErrorValue(
          ErrorValue.Type.Error,
...
class NotOperatorHandler implements OperatorHandler {
  ...
  public Value run(
    ...
    if (
      firstAtomValue.type != PrimitiveValue.Type.Bool ||
      !(firstAtomValue.value instanceof Boolean)
    ) {
      return new ErrorValue(
        ErrorValue.Type.Error,
        "not: argument should be a boolean"
      );
    }
...
class GreaterThanOrEqualOperatorHandler implements OperatorHandler {
  ...
  public Value run(
    ...
    return OperatorProvider.performNumberOperationOnValues(
      ...
      ">=",
      (referenceValue, currentValue) ->
        referenceValue.doubleValue() > currentValue.doubleValue() ||
        referenceValue.equals(currentValue)
    );
  }
}
    ...
    switch (value.type) {
      ...
      case Double:
        ...
        return new PrimitiveValue(
          OperatorProvider.numberToString((Number) value.value)
        );
      default:
        return new ErrorValue(
          ErrorValue.Type.Error,
