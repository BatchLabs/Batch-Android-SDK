...
public class UserDataBuiltinActionRunnable implements UserActionRunnable {
  ...
  public static String IDENTIFIER =
    ActionModule.RESERVED_ACTION_IDENTIFIER_PREFIX + "user.tag";
  ...
  public void performAction(
    ...
    try {
      ...
      if (collection == null) {
        Logger.internal(
          TAG,
          "Could not perform tag edit action : collection's null"
        );
        return;
      }
      ...
      if (collection.length() == 0) {
        Logger.internal(
          TAG,
          "Could not perform tag edit action : collection name is empty"
        );
        return;
      }
      ...
      if (tag.length() == 0) {
        Logger.internal(
          TAG,
          "Could not perform tag edit action : tag name is empty"
        );
        return;
      }
      ...
      if (action == null) {
        Logger.internal(
          TAG,
          "Could not perform tag edit action : action's null"
        );
        return;
      }
      ...
      if (action.equals("add")) {
        Logger.internal(
          TAG,
          "Adding tag " + tag + " to collection " + collection
        );
        ...
        Logger.internal(
          TAG,
          "Removing tag " + tag + " to collection " + collection
        );
        BatchUserDataEditor editor = Batch.User.editor();
        ...
        Logger.internal(
          TAG,
          "Could not perform tag edit action: Unknown action"
        );
      }
