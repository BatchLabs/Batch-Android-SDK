...
public class FixedRatioFrameLayout extends FrameLayout {
  ...
  public void setTargetSize(@Nullable Size2D targetSize) {
    if (
      targetSize != null && (targetSize.width <= 1 || targetSize.height <= 1)
    ) {
      targetSize = null;
    }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    double scaledWidth =
      measuredHeight * ((double) targetSize.width / targetSize.height);
    double scaledHeight =
      measuredWidth * ((double) targetSize.height / targetSize.width);
