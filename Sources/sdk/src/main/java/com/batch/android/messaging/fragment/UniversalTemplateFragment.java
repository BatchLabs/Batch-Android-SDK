...
public class UniversalTemplateFragment
  ...
  public static UniversalTemplateFragment newInstance(
    BatchMessage payloadMessage,
    UniversalMessage messageModel
  ) {
  ...
  public View onCreateView(
    ...
    if (messageModel.videoURL != null) {
      ...
      if (mediaPlayer == null) {
        ...
        try {
          ...
          Logger.internal(
            TAG,
            "Error while creating the MediaPlayer for URL " +
            messageModel.videoURL,
            e
          );
        }
    ...
    if (
      ...
      v.setSystemUiVisibility(
        v.getSystemUiVisibility() | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
      );
    }

    // Start downloading the image here
    if (
      messageModel.videoURL == null &&
      messageModel.heroImageURL != null &&
      heroDownloadTask == null
    ) {
      // Check if image is in cache
      AsyncImageDownloadTask.Result cachedResult = get(
        messageModel.heroImageURL
      );
      if (cachedResult != null) {
        displayImage(cachedResult);
      } else {
        heroDownloadTask = new AsyncImageDownloadTask(this);
        heroDownloadTask.executeOnExecutor(
