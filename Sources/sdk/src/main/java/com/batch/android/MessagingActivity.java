...
public class MessagingActivity
  ...
  implements DialogEventListener {
  ...
  private BroadcastReceiver dismissReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      if (
        intent != null &&
        ACTION_DISMISS_INTERSTITIAL.equalsIgnoreCase(intent.getAction())
      ) {
        finish();
      }
    }
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (
      ...
      !savedInstanceState.getBoolean(ROTATED, false)
    ) {
    ...
    LocalBroadcastManager
      ...
      .registerReceiver(
        dismissReceiver,
        new IntentFilter(ACTION_DISMISS_INTERSTITIAL)
      );
  }
  ...
  private boolean showMessage(BatchMessage message) {
    ...
    try {
      ...
      if (df instanceof ListenableDialog) {
        ...
        Logger.error(
          TAG,
          "Unknown error while showing Batch Message (code -1)"
        );
        return false;
      }
      ...
      Logger.error(
        TAG,
        "Unknown error while showing Batch Message (code -2)",
        e
      );
    }
  ...
  public static void startActivityForMessage(Context c, BatchMessage message) {
    ...
    LocalBroadcastManager
      .getInstance(c)
      .sendBroadcast(new Intent(ACTION_DISMISS_INTERSTITIAL));
  }
}
