...
class WebserviceCryptor {
  ...
  public byte[] decryptData(
    ...
    if (dataString.length() <= 8) {
      throw new IllegalArgumentException(
        "The data as string should be at least 9 char long"
      );
    }
   ...
   * @return
   */
  private byte[] decryptDataForVersion(
    String version,
    String publicKey,
    String data,
    Webservice ws
    ...
    if ("2".equals(version)) {
      Cryptor cryptor = CryptorFactory.getCryptorForType(
        cryptorType,
        buildKeyV2(publicKey, ws)
      );
      return cryptor.decryptToByte(data);
    } else {
      Cryptor cryptor = CryptorFactory.getCryptorForType(
        CryptorType.EAS_BASE64,
        buildKey(publicKey, ws)
      );
      return cryptor.decryptToByte(data);
    }
  }
  ...
  public byte[] encryptData(byte[] data, Webservice ws) throws Exception {
    ...
    if (ws.isDowngradedCipher) {
      ...
      cryptor =
        CryptorFactory.getCryptorForType(cryptorType, buildKey(publicKey, ws));
    } else {
      publicKey = generatePublicKey(VERSION, ws);
      cryptor =
        CryptorFactory.getCryptorForType(
          cryptorType,
          buildKeyV2(publicKey, ws)
        );
    }
    ...
    String result =
      publicKey + cryptor.encrypt(ByteArrayHelper.getUTF8String(data));
  ...
  private byte[] buildPrivateKey(Webservice ws) {
    ...
    byte[] priv = CryptorFactory
      .getCryptorForType(CryptorType.EAS_BASE64)
