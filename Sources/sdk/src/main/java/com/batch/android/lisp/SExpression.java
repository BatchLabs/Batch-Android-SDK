  ...
  public Value reduce(EvaluationContext context) {
    ...
    if (!(firstValue instanceof OperatorValue)) {
      return new ErrorValue(
        ...
        "S-Expressions should have an operator as their first value"
      );
    }

    ArrayList<PrimitiveValue> arguments = new ArrayList<>(
    ...
    for (int i = 1; i < values.size(); i++) {
      ...
      if (val == null) {
        return new ErrorValue(
          ...
          "Unexpected nil value while reducing S-Expression"
        );
      }
    ...
    if (operator == null) {
      return new ErrorValue(
        ErrorValue.Type.Internal,
        "Can't reduce a S-Expression with a nil operator"
      );
    }
