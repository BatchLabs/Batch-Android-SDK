...
public abstract class CacheHelper {
  ...
  private static List<File> filterCachedFiles(File[] files) {
    ...
    for (File file : files) {
      ...
      if (
        timestamp != null &&
        timestamp + MAX_AGE_FROM_CACHE <= System.currentTimeMillis() / 1000L
      ) {
    ...
    List<Map.Entry<File, Long>> sortedFile = new LinkedList<>(
      cachedFiles.entrySet()
    );
    Collections.sort(
      sortedFile,
    ...
    for (
      int i = 0;
      i < sortedFile.size() && i < MAX_READ_RECEIPT_FROM_CACHE;
      ++i
    ) {
