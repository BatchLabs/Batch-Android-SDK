...
public class PushImageCache {
   ...
   * @return
   */
  private static String getFilePathForIdentifier(
    Context context,
    String identifier
  ) {
   ...
   * @param image
   */
  public static void storeImageInCache(
    Context context,
    String identifier,
    Bitmap image
  ) {
    ...
    try (FileOutputStream out = new FileOutputStream(file)) {
      ...
      Logger.internal(
        TAG,
        "Error while storing push image in cache (" + identifier + ")",
        e
      );
    }
  }
  ...
  public static String buildIdentifierForURL(String url) {
    try {
      ...
      Logger.internal(
        TAG,
        "Error while computing MD5 identifier for url : " + url,
        e
      );
      return null;
    }
  ...
  private static void clearImagesIfNeeded(Context context) {
    ...
    for (int i = 0; i < files.length - (MAX_IMAGES_STORED + 1); i++) {
      ...
      Logger.internal(
        ...
        ") cause we are reaching the push image cache limit"
      );
      file.delete();
    }
  }
