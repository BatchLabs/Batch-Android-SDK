 ...
 */
@PublicSDK
public class BatchInAppMessage
  extends BatchMessage
  implements InAppMessageUserActionSource {
  ...
  static BatchInAppMessage getInstanceFromBundle(@NonNull Bundle bundle) {
    ...
    try {
      ...
      Logger.internal(
        MessagingModule.TAG,
        "Unexpected error while reading a BatchInAppMessage from a bundle",
        e
      );
  ...
  protected Bundle getBundleRepresentation() {
    ...
    b.putString(
      LANDING_PAYLOAD_KEY,
      landingPayload == null ? "{}" : landingPayload.toString()
    );
    b.putString(
      CUSTOM_PAYLOAD_KEY,
      customPayload == null ? "{}" : customPayload.toString()
    );
      ...
      if (landingPayload != null) {
        try {
          ...
          if (msg instanceof AlertMessage) {
            ...
            cachedContent =
              new BatchInterstitialContent((UniversalMessage) msg);
