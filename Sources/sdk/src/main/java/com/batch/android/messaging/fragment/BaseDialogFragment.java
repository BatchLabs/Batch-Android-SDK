...
public abstract class BaseDialogFragment<T extends Message>
  ...
  private WeakReference<DialogEventListener> eventListener = new WeakReference<>(
    null
  );
  ...
  protected void setMessageArguments(
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (getArguments() != null) {
      ...
      if (messageModel != null && payloadMessage != null) {
        analyticsDelegate =
          MessagingAnalyticsDelegateProvider.get(
            getMessageModel(),
            getPayloadMessage()
          );
    ...
    if (analyticsDelegate != null) {
      ...
      Logger.internal(
        TAG,
        "Could not create analytics delegate from arguments"
      );
    }
    ...
    if (savedInstanceState != null) {
      autoCloseAtUptime =
        savedInstanceState.getLong(
          STATE_AUTOCLOSE_TARGET_UPTIME_KEY,
          autoCloseAtUptime
        );
    }
  }
  ...
  protected BatchMessage getPayloadMessage() {
    try {
      ...
      Logger.internal(
        TAG,
        "Error while reading payload message from fragment arguments",
        e
      );
    }
  ...
  protected T getMessageModel() {
    if (messageModel == null) {
      try {
        messageModel =
          (T) getArguments().getSerializable(BUNDLE_KEY_MESSAGE_MODEL);
