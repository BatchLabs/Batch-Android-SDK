    ...
    public void scheduleDisplayReceipt(Context context, @NonNull InternalPushData pushData)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && dmi >= 0) {
            try {
                ...
                JobInfo.Builder builder = new JobInfo.Builder(JobHelper.generateUniqueJobId(
                        scheduler),
    ...
    public static synchronized void sendReceipt(Context context, boolean replay)
    {
        ...
        if (files != null && files.size() > 0) {
            ...
            DisplayReceiptPostDataProvider dataProvider = new DisplayReceiptPostDataProvider(
                    payloads.values());
            TaskRunnable runnable = WebserviceLauncher.initDisplayReceiptWebservice(context,
                    dataProvider,
