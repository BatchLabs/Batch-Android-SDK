...
public class DisplayReceiptModule extends BatchModule {
  ...
  public void batchDidStart() {
    ...
    if (Boolean.TRUE.equals(optOutModule.isOptedOutSync(context))) {
      Logger.internal(
        TAG,
        "Batch is opted out, refusing to send display receipt."
      );
      return;
    }
   ...
   */
  public void scheduleDisplayReceipt(
    Context context,
    @NonNull InternalPushData pushData
  ) {
    if (Boolean.TRUE.equals(optOutModule.isOptedOutSync(context))) {
      Logger.internal(
        TAG,
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && dmi >= 0) {
      try {
        ...
        if (
          scheduler.schedule(builder.build()) == JobScheduler.RESULT_FAILURE
        ) {
          Logger.internal(TAG, "Failed to schedule the display receipt job");
        } else {
          Logger.internal(
            TAG,
            "Successfully scheduled the display receipt job"
          );
        }
  ...
  public static synchronized void sendReceipt(Context context, boolean replay) {
    ...
    if (files != null && files.size() > 0) {
      ...
      DisplayReceiptPostDataProvider dataProvider = new DisplayReceiptPostDataProvider(
        payloads.values()
      );
