...
public class MessagingModule extends BatchModule {
   ...
   */
  public static final String ACTION_DISMISS_INTERSTITIAL =
    "com.batch.android.messaging.DISMISS_INTERSTITIAL";

  /**
   * Broadcast this action using {@link LocalBroadcastManager} and banners will dismiss themselves
   */
  public static final String ACTION_DISMISS_BANNER =
    "com.batch.android.messaging.DISMISS_BANNER";
  ...
  private static final String MESSAGING_EVENT_NAME_GLOBAL_TAP =
    "global_tap_action";

  private static final String MESSAGING_EVENT_NAME_CTA = "cta_action";

  private static final String MESSAGING_EVENT_NAME_WEBVIEW_CLICK =
    "webview_click";
  ...
  public void setTypefaceOverride(
  ...
  public DialogFragment loadFragment(
    ...
    if (message instanceof AlertMessage) {
      return AlertTemplateFragment.newInstance(
        payloadMessage,
        (AlertMessage) message
      );
    } else if (message instanceof UniversalMessage) {
      return UniversalTemplateFragment.newInstance(
        payloadMessage,
        (UniversalMessage) message
      );
    } else if (message instanceof ModalMessage) {
      return ModalTemplateFragment.newInstance(
        payloadMessage,
        (ModalMessage) message
      );
    } else if (message instanceof ImageMessage) {
      return ImageTemplateFragment.newInstance(
        payloadMessage,
        (ImageMessage) message
      );
    } else if (message instanceof WebViewMessage) {
      return WebViewTemplateFragment.newInstance(
        payloadMessage,
    ...
    if (presentedActivity != null) {
      ...
      if (listener instanceof Messaging.LifecycleListener2) {
        shouldDisplayMessage =
          !((Messaging.LifecycleListener2) listener).onBatchInAppMessageReady(
              message
            );
      }
  ...
  private JSONObject generateBaseEventParameters(
  ...
  private void trackGenericEvent(
    @NonNull Message message,
    @NonNull String type
  ) {
    try {
      trackerModule.track(
        InternalEvents.MESSAGING,
        generateBaseEventParameters(message, type)
      );
  ...
  private void trackCloseErrorEvent(
    @NonNull Message message,
    @NonNull MessagingError cause
  ) {
    try {
      final JSONObject parameters = generateBaseEventParameters(
        message,
  ...
  private void trackWebViewClickEvent(
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Error while tracking a webview trackClick event",
        e
      );
    }
  }
  ...
  public void onMessageGlobalTap(
    @NonNull Message message,
    @NonNull Action action
  ) {
  ...
  public void onMessageAutoClosed(@NonNull Message message) {
    ...
    if (listener != null) {
      listener.onBatchMessageCancelledByAutoclose(
        message.devTrackingIdentifier
      );
    }
  }
  ...
  public void onMessageClosedError(
    @NonNull Message message,
    @NonNull MessagingError cause
  ) {
