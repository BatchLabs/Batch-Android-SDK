...
{
    public final static String TAG = "Messaging";
    //region: Constants
    ...
    private MessagingModule(ActionModule actionModule,
    ...
    public void setTypefaceOverride(@Nullable Typeface normalTypeface,
    ...
    public DialogFragment loadFragment(@NonNull Context context,
                                       @NonNull BatchMessage payloadMessage,
                                       @NonNull JSONObject json) throws BatchMessagingException
    {
        ...
        if (message instanceof AlertMessage) {
            ...
            return UniversalTemplateFragment.newInstance(payloadMessage,
                    (UniversalMessage) message);
        } else if (message instanceof ModalMessage) {
            return ModalTemplateFragment.newInstance(payloadMessage,
                    (ModalMessage) message);
        } else if (message instanceof ImageMessage) {
            return ImageTemplateFragment.newInstance(payloadMessage,
                    (ImageMessage) message);
        } else if (message instanceof WebViewMessage) {
            return WebViewTemplateFragment.newInstance(payloadMessage,
    ...
    public BatchBannerView loadBanner(@NonNull Context context,
                                      @NonNull BatchMessage payloadMessage,
                                      @NonNull JSONObject json) throws BatchMessagingException
    {
        //noinspection ConstantConditions
        ...
        if (message instanceof BannerMessage) {
            return BatchBannerViewPrivateHelper.newInstance(payloadMessage,
                    (BannerMessage) message,
                    MessagingAnalyticsDelegateProvider.get(message, payloadMessage));
        } else {
            throw new BatchMessagingException(
                    "The BatchMessage instance does not represent a banner.");
        }
    }
    ...
    public void displayMessage(@NonNull Context context,
                               ...
                               boolean bypassDnD)
    {
        if (!bypassDnD && doNotDisturbMode) {
            ...
            Logger.info(TAG,
                    "Enqueuing a message, as it should have been displayed but Do Not Disturb is enabled.");
            return;
        }
    ...
    public void displayInAppMessage(@NonNull BatchInAppMessage message)
    {
        ...
        if (presentedActivity != null) {
            ...
            if (listener instanceof Messaging.LifecycleListener2) {
                shouldDisplayMessage = !( (Messaging.LifecycleListener2) listener ).onBatchInAppMessageReady(
                        message);
            }
            ...
            if (shouldDisplayMessage) {
                ...
                Logger.internal(LocalCampaignsModule.TAG,
                        "Developer prevented automatic In-App display");
            }
        } else {
            Logger.error(LocalCampaignsModule.TAG,
    ...
    private JSONObject generateBaseEventParameters(@NonNull Message message,
    ...
    {
        try {
            trackerModule.track(InternalEvents.MESSAGING,
                    generateBaseEventParameters(
                            message,
                            type));
    ...
    {
        try {
            final JSONObject parameters = generateBaseEventParameters(message,
                    MESSAGING_EVENT_NAME_CLOSE_ERROR);
            if (cause != null) {
    ...
    private void trackWebViewClickEvent(@NonNull Message message,
                                        @NonNull String actionName,
                                        @Nullable String buttonAnalyticsID)
    {
        try {
            final JSONObject parameters = generateBaseEventParameters(message,
                    MESSAGING_EVENT_NAME_WEBVIEW_CLICK);
    ...
    public void onMessageCTAClicked(@NonNull Message message, int ctaIndex, @NonNull CTA cta)
    {
        ...
        if (listener != null) {
            listener.onBatchMessageActionTriggered(message.devTrackingIdentifier,
                    ctaIndex,
    ...
    public void onWebViewMessageClickTracked(@NonNull Message message,
                                             ...
                                             @Nullable String buttonAnalyticsId)
    {
        ...
        if (listener != null) {
            listener.onBatchMessageWebViewActionTriggered(message.devTrackingIdentifier,
                    buttonAnalyticsId,
    ...
    public void onMessageGlobalTap(@NonNull Message message, @NonNull Action action)
    {
        ...
        try {
            final JSONObject parameters = generateBaseEventParameters(message,
                    MESSAGING_EVENT_NAME_GLOBAL_TAP);
            parameters.put("action", actionName);
            trackerModule.track(InternalEvents.MESSAGING, parameters);
        } catch (JSONException e) {
            Logger.internal(TAG, "Error while tracking CTA event", e);
