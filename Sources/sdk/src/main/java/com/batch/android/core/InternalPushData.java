...
public class InternalPushData {
   ...
   */
  private static final String OLD_BIG_PICTURE_ICON_BEHAVIOUR =
    "old_bp_icnhandling";
  ...
  public InternalPushData(String batchData) {
    if (batchData == null || batchData.isEmpty()) {
      throw new NullPointerException(
        "Cannot init PushData without the associated JSON data"
      );
    }
  ...
  public static InternalPushData getPushDataForReceiverIntent(
  ...
  public static InternalPushData getPushDataForReceiverBundle(
  ...
  public static InternalPushData getPushDataForFirebaseMessage(
    @Nullable RemoteMessage message
  ) {
    if (message == null) {
      return null;
    }
  ...
  public boolean isLocalCampainsRefresh() {
    try {
      return (
        payload.has(IS_LOCAL_CAMPAIGNS_REFRESH_KEY) &&
        payload.getBoolean(IS_LOCAL_CAMPAIGNS_REFRESH_KEY)
      );
    } catch (JSONException e) {
      return false;
    }
  ...
  public List<BatchNotificationAction> getActions() {
    ...
    if (actions != null) {
      for (int i = 0; i < actions.length(); i++) {
        ...
        tmpAction.shouldDismissNotification =
          jsonAction.reallyOptBoolean("d", true);
        ...
        if (TextUtils.isEmpty(tmpAction.label)) {
          Logger.internal(
            PushModule.TAG,
            "InternalPushData - getActions: Empty or null label. Skipping."
          );
          continue;
        }
  ...
  public Priority getPriority() {
    ...
    if (!TextUtils.isEmpty(priority)) {
      ...
      try {
        ...
        Logger.internal(
          PushModule.TAG,
          "Error while reading the priority number " + priority,
          e
        );
      }
    }
  ...
  public Map<String, Object> getExtraParameters() {
    ...
    try {
      ...
      Logger.internal(
        PushModule.TAG,
        "Error while deserializing the PushData extra parameters.",
        e
      );
    }
  ...
  public Map<String, Object> getOpenData() {
    try {
      final JSONObject trimmedPayload = new JSONObject(
        payload,
        new String[] { OPEN_DATA_KEY }
      );
      return JSONHelper.jsonObjectToMap(trimmedPayload);
    } catch (JSONException e) {
      Logger.internal(
        PushModule.TAG,
        "Error while deserializing the PushData open data.",
        e
      );
    }
  ...
  public Map<String, Object> getReceiptEventData() {
    try {
      ...
      Logger.internal(
        PushModule.TAG,
        "Error while deserializing the receipt event data.",
        e
      );
    }
