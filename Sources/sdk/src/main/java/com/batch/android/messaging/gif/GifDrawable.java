...
public class GifDrawable extends Drawable implements Animatable
{
    ...
    private Executor frameProducerExecutor = Executors.newSingleThreadExecutor(new NamedThreadFactory(
            "gif"));
    ...
    private void produceNextFrame()
    {
        ...
        try {
            gifDecoder.advance();
            FrameInfo nextFrame = new FrameInfo(gifDecoder.getNextFrame(),
    ...
    private void requestNewFrameIfNeeded()
    {
        ...
        if (now > nextFrameDeadline) {
            ...
            if (nextFrame != null) {
                ...
                nextFrameDeadline = Math.max(now + 1,
                        now + currentFrame.delay - Math.abs(now - nextFrameDeadline)); // If we're late, deduce the time that's already elapsed from the next deadline. Make sure we don't ask for a deadline in the past
