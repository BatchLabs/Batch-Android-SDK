...
public class GifDrawable extends Drawable implements Animatable {
  ...
  private Executor frameProducerExecutor = Executors.newSingleThreadExecutor(
    new NamedThreadFactory("gif")
  );
  ...
  private void produceNextFrame() {
    ...
    try {
      gifDecoder.advance();
      FrameInfo nextFrame = new FrameInfo(
        gifDecoder.getNextFrame(),
        gifDecoder.getNextDelay()
      );
      Message
        .obtain(mainThreadHandler, MESSAGE_FRAME_PRODUCED, nextFrame)
        .sendToTarget();
    } catch (OutOfMemoryError e) {
      //TODO: Batch log
      ranOutOfMemory = true;
      Message
        .obtain(mainThreadHandler, MESSAGE_RAN_OUT_OF_MEMORY)
        .sendToTarget();
    ...
    if (now > nextFrameDeadline) {
      ...
      if (nextFrame != null) {
        ...
        nextFrameDeadline =
          Math.max(
            now + 1,
            now + currentFrame.delay - Math.abs(now - nextFrameDeadline)
          ); // If we're late, deduce the time that's already elapsed from the next deadline. Make sure we don't ask for a deadline in the past
