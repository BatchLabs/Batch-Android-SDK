...
public abstract class GCMAbstractRegistrationProvider implements PushRegistrationProvider
{
    ...
    public void checkLibraryAvailability() throws PushRegistrationProviderAvailabilityException
    {
        ...
        if (!isC2DMessagePermissionAvailable()) {
            throw new PushRegistrationProviderAvailabilityException(
                    "Batch.Push : Permission C2D_MESSAGE is missing.");
        }

        if (!isWakeLockPermissionAvailable()) {
            throw new PushRegistrationProviderAvailabilityException(
                    "Batch.Push : Permission WAKE_LOCK is missing.");
        }
    }
    ...
    {
        try {
            return GenericHelper.checkPermission("com.google.android.c2dm.permission.RECEIVE",
                    context);
        } catch (Exception e) {
            Logger.error(TAG,
                    "Error while checking com.google.android.c2dm.permission.RECEIVE permission",
                    e);
            return false;
        }
    }
    ...
    {
        try {
            return GenericHelper.checkPermission(context.getPackageName() + ".permission.C2D_MESSAGE",
                    context);
        } catch (Exception e) {
    ...
    {
        try {
            return GenericHelper.checkPermission("android.permission.WAKE_LOCK",
                    context);
        } catch (Exception e) {
