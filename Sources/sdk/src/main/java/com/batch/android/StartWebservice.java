...
class StartWebservice extends BatchQueryWebservice implements TaskRunnable {
  ...
  protected List<Query> getQueries() {
    ...
    queries.add(
      new StartQuery(applicationContext, fromPush, pushId, userActivity)
    );

    // Add push query if we have a push token
    Registration registration = PushModuleProvider
      .get()
      .getRegistration(applicationContext);
  ...
  public void run() {
    try {
      ...
      try {
        ...
        Logger.internal(
          TAG,
          "Error on StartWebservice : " + error.getReason().toString(),
          error.getCause()
        );
       ...
       */
      StartResponse startResponse = getResponseFor(
        StartResponse.class,
        QueryType.START
      );
