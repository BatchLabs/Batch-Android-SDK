...
public class Document {
  ...
  @NonNull
  public Map<String, String> getFlatRules(
    @NonNull DOMNode node,
    @Nullable Point screenSize
  ) {
  ...
  @NonNull
  public Map<String, String> getFlatRules(
    @Nullable List<Declaration> declarations
  ) {
  ...
  @NonNull
  public List<Declaration> getRules(
    @NonNull DOMNode node,
    @Nullable Point screenSize
  ) {
    final List<Declaration> declarations = new ArrayList<>(
      getRules(node, rulesets)
    );
  ...
  @NonNull
  private List<Declaration> getRules(
    @NonNull DOMNode node,
    @NonNull List<Ruleset> rulesets
  ) {
    final List<Declaration> declarations = new ArrayList<>();
    ...
    if (screenSize != null) {
      ...
      if (matcher.matches()) {
        try {
          ...
          Logger.internal(
            TAG,
            "Error while parsing a media query size rule",
            e
          );
          return false;
        }
    ...
    if (mediaQueryRule.startsWith("@android")) {
      if (mediaQueryRule.length() == 8) { // It's only @android
        ...
        if (mediaQueryRule.length() > 9) { // It's @android-xxxx
          final String wantedApiLevel = mediaQueryRule.substring(
            9,
            mediaQueryRule.length()
          );
          try {
