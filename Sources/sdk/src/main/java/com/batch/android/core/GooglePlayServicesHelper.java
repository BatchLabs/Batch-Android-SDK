...
public final class GooglePlayServicesHelper {
   ...
   */
  public static String getGooglePlayServicesAvailabilityString(
  ...
  public static Integer getGooglePlayServicesAvailabilityInteger(
    Context context
  ) {
    ...
    try {
      ...
      Logger.error(
        PushModule.TAG,
        "Error while retreiving Google Play Services lib availability",
        e
      );
      return null;
    }
  ...
  public static Integer getGooglePlayServicesLibVersion(Context context) {
    ...
    try {
      ...
      Logger.error(
        PushModule.TAG,
        "Error while retreiving Google Play Services lib version",
        e
      );
      return null;
    } finally {
  ...
  public static Integer isPushAvailable(Context context) {
    ...
    if (
      libVersion == null ||
      libVersion < PUSH_ID_VERSION ||
      !ReflectionHelper.isGMSGoogleCloudMessagingPresent()
    ) {
      return null;
    }
  ...
  public static String getPushToken(Context context, String senderID) {
    try {
      ...
      if (isInvalidSenderException(e)) {
        Logger.error(
          PushModule.TAG,
          "GCM sender id is invalid. Please check your GCM configuration. More info: " +
        ...
        if (e.getCause() != null) {
          Logger.error(
            PushModule.TAG,
            "Error while requesting push token to GCM : " +
  ...
  public static Integer isInstanceIdPushAvailable(Context context) {
    ...
    if (
      libVersion == null ||
      libVersion < INSTANCE_ID_VERSION ||
      !ReflectionHelper.isGMSInstanceIDPresent()
    ) {
      return null;
    }
  ...
  public static String getInstancePushToken(Context context, String senderID) {
    try {
      ...
      Method singletonMethod = clazz.getMethod("getInstance", Context.class);
      Method registerMethod = clazz.getMethod(
        "getToken",
        String.class,
        String.class
      );
      ...
      Logger.internal(
        PushModule.TAG,
        "Error while registering for instance id push",
        e
      );
      ...
      if (cause instanceof IOException) {
        ...
        Logger.error(
          PushModule.TAG,
          "Could not get GCM Instance ID: " + publicErrorReason
        );
      }
      return null;
    }
