...
// -------------------------------------------->
                         ...
                         String... parameters) throws MalformedURLException
    {
        ...
        this.isDowngradedCipher = ParametersProvider.get(applicationContext).get(ParameterKeys.WS_CIPHERV2_LAST_FAILURE_KEY) != null;
    }
...
// -------------------------------------------->
    ...
    private Cryptor getCryptor()
    {
        ...
        String cryptorType = ParametersProvider.get(applicationContext).get(
                cryptorTypeParamKey);
        if (cryptorType == null) {
            return null;
        }
    ...
    private CryptorMode getGetCryptorMode()
    {
        ...
        if (cryptorModeParamKey != null) {
            String cryptorMode = ParametersProvider.get(applicationContext).get(
                    cryptorModeParamKey);
            if (cryptorMode != null) {
                try {
...
// -------------------------------------------->
        ...
        do {
            ...
            try {
                ...
                if (isResponseValid(errorCode)) {
                    ...
                    error = new WebserviceError(getResponseErrorCause(connection.getResponseCode()),
    ...
    protected JSONObject getStandardResponseBodyIfValid() throws WebserviceError, JSONException
    {
        ...
        if (header.has("ts") && !header.isNull("ts")) {
            ...
            String currentTSString = ParametersProvider.get(applicationContext).get(
                    ParameterKeys.SERVER_TIMESTAMP);
            if (currentTSString != null) // If we already have a ts, be sure the new one is >
            {
                long currentTS = Long.parseLong(currentTSString);
                if (currentTS < ts) {
                    ParametersProvider.get(applicationContext).set(ParameterKeys.SERVER_TIMESTAMP,
                            Long.toString(ts),
                            true);
                } else {
                    ts = currentTS;
...
// --------------------------------------------->
    ...
    private int getConnectTimeout()
    {
        ...
        return Integer.parseInt(ParametersProvider.get(applicationContext).get(
                ParameterKeys.DEFAULT_CONNECT_TIMEOUT_KEY));
    }
    ...
    private int getReadTimeout()
    {
        ...
        return Integer.parseInt(ParametersProvider.get(applicationContext).get(
                ParameterKeys.DEFAULT_READ_TIMEOUT_KEY));
    }
    ...
    private int getMaxRetryCount()
    {
        ...
        return Integer.parseInt(ParametersProvider.get(applicationContext).get(
                ParameterKeys.DEFAULT_RETRY_NUMBER_KEY));
    }
