...
public abstract class Webservice {
  ...
  private PatternURLSorter getURLSorter() {
    ...
    String pattern = ParametersProvider
      .get(applicationContext)
      .get(parameterKey);
    if (pattern == null) {
      return null;
  ...
  private Cryptor getCryptor() {
    ...
    String cryptorType = ParametersProvider
      .get(applicationContext)
      .get(cryptorTypeParamKey);
    if (cryptorType == null) {
      return null;
    }

    if (isDowngradedCipher) {
      return CryptorFactory.getCryptorForType(
        CryptorFactory.CryptorType.EAS_BASE64
      );
    }
  ...
  private CryptorMode getGetCryptorMode() {
    ...
    if (cryptorModeParamKey != null) {
      String cryptorMode = ParametersProvider
        .get(applicationContext)
        .get(cryptorModeParamKey);
      if (cryptorMode != null) {
        try {
          CryptorMode value = CryptorMode.fromValue(
            Integer.parseInt(cryptorMode)
          );
          if (value != null) {
            return value;
          }
          ...
          Logger.internal(
            TAG,
            "Error while getting cryptor mode for key : " + cryptorModeParamKey,
            e
          );
        }
      }
  ...
  protected JSONObject getStandardResponseBodyIfValid()
    ...
    if (header.has("ts") && !header.isNull("ts")) {
      ...
      String currentTSString = ParametersProvider
        .get(applicationContext)
  ...
  public static String encode(String part) {
    try {
      ...
      throw new IllegalStateException(
        "UTF-8 encoding is not supported, can't build URL"
      );
    }
  }
  ...
  protected HttpURLConnection buildConnection() throws Exception {
     ...
     * Create connection
     */
    HttpURLConnection connection = (HttpURLConnection) buildURL()
      .openConnection();
    ...
    if (Parameters.ENABLE_WS_INTERCEPTOR && wsInterceptor != null) {
      ...
      wsInterceptor.onPreConnect(
        id,
        connection,
        data,
        postContent != null ? postContent.length : 0
      );
    }
   ...
   * @return
   */
  private byte[] buildPostParameters(PostDataProvider<?> provider)
    throws Exception {
  ...
  private void enabledDowngradedMode() {
    ...
    ParametersProvider
      .get(applicationContext)
      .set(ParameterKeys.WS_CIPHERV2_LAST_FAILURE_KEY, now, true);
  }

  protected void addRequestSignatures(
  ...
  protected String getSignatureBody(
    HttpURLConnection connection,
    List<String> keys
  ) {
  ...
  private int getConnectTimeout() {
    ...
    return Integer.parseInt(
      ParametersProvider
        .get(applicationContext)
        .get(ParameterKeys.DEFAULT_CONNECT_TIMEOUT_KEY)
    );
  }
  ...
  private int getReadTimeout() {
    ...
    return Integer.parseInt(
      ParametersProvider
        .get(applicationContext)
        .get(ParameterKeys.DEFAULT_READ_TIMEOUT_KEY)
    );
  }
  ...
  private int getMaxRetryCount() {
    ...
    return Integer.parseInt(
      ParametersProvider
        .get(applicationContext)
        .get(ParameterKeys.DEFAULT_RETRY_NUMBER_KEY)
    );
  }
  ...
  interface Interceptor {
    HttpURLConnection onBuildHttpConnection(
