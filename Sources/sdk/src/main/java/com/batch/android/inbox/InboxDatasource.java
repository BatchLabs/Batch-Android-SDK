...
public final class InboxDatasource {
  ...
  public List<InboxNotificationContentInternal> getNotifications(
    ...
    try (Cursor cursor = database.rawQuery(query, args)) {
      while (cursor.moveToNext()) {
        InboxNotificationContentInternal notification = parseNotification(
          cursor
        );
    ...
    try (
      ...
      if (cursor.moveToFirst()) {
        return cursor.getLong(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_DATE)
        );
      }
    } catch (Exception e) {
  ...
  public long getFetcherID(FetcherType type, String identifier) {
    ...
    if (database == null) {
      Logger.internal(
        TAG,
        "Attempted to get/insert a fetcher to a closed database"
      );
      database = databaseHelper.getWritableDatabase();
    }
    ...
    try {
      ...
      try (
        ...
        if (cursor.moveToFirst()) {
          return cursor.getLong(
            cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_DB_ID)
          );
        }
      } catch (Exception ex) {
  ...
  public boolean insertResponse(
    ...
    for (InboxNotificationContentInternal notification : response.notifications) {
      Logger.internal(
        TAG,
        "Add notification in DB: " + notification.identifiers.identifier
      );
  ...
  protected boolean insert(
    InboxNotificationContentInternal notification,
    long fetcherId
  ) {
    if (database == null) {
      Logger.internal(
        TAG,
        "Attempted to insert a notification to a closed database"
      );
      database = databaseHelper.getWritableDatabase();
    }
    ...
    try {
      ...
      values.put(
        InboxDatabaseHelper.COLUMN_SEND_ID,
        notification.identifiers.sendID
      );
      ...
      linkValues.put(
        InboxDatabaseHelper.COLUMN_CUSTOM_ID,
        notification.identifiers.customID
      );
  ...
  private InboxNotificationContentInternal parseNotification(Cursor cursor) {
    try {
      final JSONObject payload = new JSONObject(
        cursor.getString(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_PAYLOAD)
        )
      );

      final InternalPushData batchData = new InternalPushData(
        payload.getString("com.batch")
      );
      final NotificationIdentifiers identifiers = new NotificationIdentifiers(
        ...
        cursor.getString(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_SEND_ID)
        )
      );

      identifiers.customID =
        cursor.getString(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_CUSTOM_ID)
        );
      identifiers.installID =
        cursor.getString(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_INSTALL_ID)
        );
      ...
      c.isUnread =
        cursor.getInt(
          cursor.getColumnIndexOrThrow(InboxDatabaseHelper.COLUMN_UNREAD)
        ) !=
        0;
   ...
   * @return
   */
  private InboxCandidateNotificationInternal parseCandidateNotification(
    Cursor cursor
  ) {
