...
// -------------------------------------------->
    ...
    public List<InboxNotificationContentInternal> getNotifications(List<String> notificationIds,
                                                                   long fetcherId)
    {
        final List<InboxNotificationContentInternal> notifications = new ArrayList<>();

        String query = "SELECT * "
                + " FROM " + InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS
                + " INNER JOIN " + InboxDatabaseHelper.TABLE_NOTIFICATIONS
    ...
    {
        try (Cursor cursor = database.query(InboxDatabaseHelper.TABLE_NOTIFICATIONS,
    ...
    public long getFetcherID(FetcherType type, String identifier)
    {
        ...
        try {
            return database.insertWithOnConflict(InboxDatabaseHelper.TABLE_FETCHERS,
                    null,
                    values,
            ...
            try (Cursor cursor = database.query(InboxDatabaseHelper.TABLE_FETCHERS,
                    new String[]{InboxDatabaseHelper.COLUMN_DB_ID},
                    InboxDatabaseHelper.COLUMN_FETCHER_TYPE + "=? and "
     ...
     * @return
     */
    public List<InboxCandidateNotificationInternal> getCandidateNotifications(@Nullable String cursor,
                                                                              int limit,
                                                                              long fetcherId)
    {
        ...
        if (!TextUtils.isEmpty(cursor)) {
            ...
            if (cursorTime != -1) {
                String query = "SELECT "
                        ...
                        + InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS + "." + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + ", "
                        ...
                        + " ON " + InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS + "." + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID
                        + " = " + InboxDatabaseHelper.TABLE_NOTIFICATIONS + "." + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID
                        + " WHERE " + InboxDatabaseHelper.COLUMN_DATE + " < ?"
                        + " AND " + InboxDatabaseHelper.COLUMN_FETCHER_ID + " = ?"
                        + " ORDER BY " + InboxDatabaseHelper.COLUMN_DATE + " DESC"
                        + " LIMIT " + limit;
                try (Cursor result = database.rawQuery(query,
                        new String[]{Long.toString(cursorTime), Long.toString(fetcherId)}))
                {
                    while (result.moveToNext()) {
                        InboxCandidateNotificationInternal candidate = parseCandidateNotification(
                                result);
    ...
    protected boolean insert(InboxNotificationContentInternal notification, long fetcherId)
    {
        ...
        try {
            final ContentValues values = new ContentValues();
            values.put(InboxDatabaseHelper.COLUMN_NOTIFICATION_ID,
                    notification.identifiers.identifier);
            ...
            values.put(InboxDatabaseHelper.COLUMN_PAYLOAD,
                    new JSONObject(notification.payload).toString());

            ContentValues linkValues = new ContentValues();
            linkValues.put(InboxDatabaseHelper.COLUMN_NOTIFICATION_ID,
                    notification.identifiers.identifier);
            linkValues.put(InboxDatabaseHelper.COLUMN_FETCHER_ID, fetcherId);
            linkValues.put(InboxDatabaseHelper.COLUMN_INSTALL_ID,
                    notification.identifiers.installID);
            linkValues.put(InboxDatabaseHelper.COLUMN_CUSTOM_ID, notification.identifiers.customID);

            database.beginTransactionNonExclusive();
            try {
                database.insertWithOnConflict(InboxDatabaseHelper.TABLE_NOTIFICATIONS,
                        null,
                        values,
                        SQLiteDatabase.CONFLICT_REPLACE);

                database.insertWithOnConflict(InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS,
                        null,
                        linkValues,
                        SQLiteDatabase.CONFLICT_REPLACE);
            ...
            for (String key : notification.keySet()) {
                ...
                switch (key) {
                    ...
                    case Batch.Push.BODY_KEY:
                        ...
                        break;
                    case "read":
                        boolean unread = !notification.reallyOptBoolean("read", false)
                    ...
                    case InboxDatabaseHelper.COLUMN_PAYLOAD:
                        ...
                        notificationsValues.put(InboxDatabaseHelper.COLUMN_PAYLOAD,
                                payload.toString());
                        break;
                    case "installId":
            ...
            try {
                ...
                if (values.size() > 0) {
                    database.update(
                            ...
                            values,
                            InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + " =? and "
                                    + InboxDatabaseHelper.COLUMN_FETCHER_ID + " =?",
                            new String[]{
                                    notificationId,
                                    Long.toString(fetcherId)
                            }
                    );
                }
    ...
    public int markAllAsRead(long time, long fetcherId)
    {
        ...
        return database.update(
                ...
                InboxDatabaseHelper.COLUMN_DATE + " <= ?"
                        + " AND EXISTS ("
                        + " SELECT " + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID
                        + " FROM " + InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS
                        + " WHERE " + InboxDatabaseHelper.COLUMN_FETCHER_ID + " = ?"
                        + " AND " + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + " = "
                        + InboxDatabaseHelper.TABLE_NOTIFICATIONS + "." + InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + ")",
                new String[]{
                        Long.toString(time),
                        Long.toString(fetcherId)
    ...
    public boolean deleteNotifications(List<String> notificationIds)
    {
        ...
        database.beginTransactionNonExclusive();
        try {
            database.delete(InboxDatabaseHelper.TABLE_NOTIFICATIONS,
                    InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + " IN (" + createInClause(args.length) + ")",
                    args
            );

            database.delete(InboxDatabaseHelper.TABLE_FETCHERS_NOTIFICATIONS,
                    InboxDatabaseHelper.COLUMN_NOTIFICATION_ID + " IN (" + createInClause(args.length) + ")",
                    args
            );
    ...
    {
        try {
            final JSONObject payload = new JSONObject(cursor.getString(cursor.getColumnIndexOrThrow(
                    InboxDatabaseHelper.COLUMN_PAYLOAD)));

            final InternalPushData batchData = new InternalPushData(payload.getString(
                    "com.batch"));
            ...
            identifiers.installID = cursor.getString(cursor.getColumnIndexOrThrow(
                    InboxDatabaseHelper.COLUMN_INSTALL_ID));
