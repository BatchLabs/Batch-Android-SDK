...
public class CountdownView extends ProgressBar implements Styleable {
  ...
  private void onAnimationFrame() {
    ...
    if (currentAnimationTime >= animationEndDate) {
      ...
      setProgress(
        (int) (
          (((float) animationEndDate - currentAnimationTime) / duration) *
          MAX_PROGRESS
        )
      );
    }
  ...
  public void setColor(@ColorInt int color) {
    ...
    setProgressDrawable(
      new ClipDrawable(d, Gravity.LEFT, ClipDrawable.HORIZONTAL)
    );
    invalidate();
  }
  ...
  public Parcelable onSaveInstanceState() {
    AnimatedCountdownSavedState state = new AnimatedCountdownSavedState(
