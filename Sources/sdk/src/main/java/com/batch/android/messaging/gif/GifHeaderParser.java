...
import static com.batch.android.messaging.gif.GifDecoder.STATUS_FORMAT_ERROR;
...
public class GifHeaderParser {
  ...
  public static GifHeader parse(@NonNull ByteBuffer data) {
    GifHeaderParser hp = new GifHeaderParser();
    hp.setData(data);
    return hp.parseHeader();
  }
  ...
  private void readGraphicControlExt() {
    ...
    header.currentFrame.dispose =
      (packed & GCE_MASK_DISPOSAL_METHOD) >> GCE_DISPOSAL_METHOD_SHIFT;
    if (header.currentFrame.dispose == DISPOSAL_UNSPECIFIED) {
      // Elect to keep old image if discretionary.
      header.currentFrame.dispose = DISPOSAL_NONE;
    }
    header.currentFrame.transparency =
      (packed & GCE_MASK_TRANSPARENT_COLOR_FLAG) != 0;
  ...
  private void readBitmap() {
    ...
    header.currentFrame.interlace =
      (packed & DESCRIPTOR_MASK_INTERLACE_FLAG) != 0;
    if (lctFlag) {
