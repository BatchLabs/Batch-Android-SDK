...
public class BatchUserDataEditor {
  ...
  static final Pattern ATTR_KEY_PATTERN = Pattern.compile(
    "^[a-zA-Z0-9_]{1,30}$"
  );
  ...
  public BatchUserDataEditor setLanguage(final @Nullable String language) {
    if (language != null && language.trim().length() < 2) {
      Logger.error(
        TAG,
        "setLanguage called with invalid language (must be at least 2 chars)"
      );
      return this;
    }
  ...
  public BatchUserDataEditor setRegion(final @Nullable String region) {
    if (region != null && region.trim().length() < 2) {
      Logger.error(
        TAG,
        "setRegion called with invalid region (must be at least 2 chars)"
      );
      return this;
    }
  ...
  public BatchUserDataEditor setIdentifier(final @Nullable String identifier) {
    if (identifier != null && identifier.trim().length() > 1024) {
      Logger.error(
        TAG,
        "setIdentifier called with invalid identifier (must be less than 1024 chars)"
      );
      return this;
    }
   ...
   * @return This object instance, for method chaining
   */
  public BatchUserDataEditor setAttribute(
    ...
    synchronized (operationQueue) {
      operationQueue.add(datasource ->
        datasource.setAttribute(normalizedKey, value)
      );
    }
  ...
  public BatchUserDataEditor setAttribute(
    ...
    if (value == null) {
      Logger.error(
        TAG,
        "setAttribute cannot be used with a null value. Ignoring attribute '" +
        key +
        "'"
      );
      return this;
    ...
    synchronized (operationQueue) {
      operationQueue.add(datasource ->
        datasource.setAttribute(normalizedKey, date)
      );
    }
   ...
   * @return This object instance, for method chaining
   */
  public BatchUserDataEditor addTag(
    ...
    synchronized (operationQueue) {
      operationQueue.add(datasource ->
        datasource.addTag(normalizedCollection, normalizedValue)
      );
    }
   ...
   * @return This object instance, for method chaining
   */
  public BatchUserDataEditor removeTag(
    final @NonNull String collection,
    final @NonNull String tag
  ) {
    final String normalizedCollection;
    ...
    synchronized (operationQueue) {
      operationQueue.add(datasource ->
        datasource.removeTag(normalizedCollection, normalizedValue)
      );
    }
   ...
   * @return This object instance, for method chaining
   */
  public BatchUserDataEditor clearTagCollection(
    final @NonNull String collection
  ) {
    synchronized (operationQueue) {
      try {
        ...
        operationQueue.add(datasource ->
          datasource.clearTags(normalizedCollection)
        );
      } catch (AttributeValidationException e) {
        Logger.error(
          TAG,
          String.format(
            "Invalid tag collection. Ignoring tag collection clear request '%s' .",
            collection
          )
        );
        return this;
  ...
  private String normalizeAttributeKey(final String key)
  ...
  private String normalizeTagCollection(final String collection)
    throws AttributeValidationException {
    if (
      TextUtils.isEmpty(collection) ||
      !ATTR_KEY_PATTERN.matcher(collection).matches()
    ) {
      throw new AttributeValidationException();
    }
  ...
  private String normalizeTagValue(final String value)
    throws AttributeValidationException {
  ...
  private UserOperation getUserUpdateOperation() {
    if (
      !updatedFields[LANGAGUE_INDEX] &&
      !updatedFields[REGION_INDEX] &&
  ...
  private List<UserOperation> popOperationQueue() {
    final List<UserOperation> pendingOperationQueue = new LinkedList<>(
      operationQueue
    );
