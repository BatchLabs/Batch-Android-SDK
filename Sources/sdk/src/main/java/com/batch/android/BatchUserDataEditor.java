...
//region Public API
    ...
    public BatchUserDataEditor addTag(final @NonNull String collection, final @NonNull String tag)
    {
        ...
        synchronized (operationQueue) {
            operationQueue.add(datasource -> datasource.addTag(normalizedCollection,
                    normalizedValue));
        }
     ...
     * @return This object instance, for method chaining
     */
    public BatchUserDataEditor removeTag(final @NonNull String collection,
                                         final @NonNull String tag)
    {
        ...
        synchronized (operationQueue) {
            operationQueue.add(datasource -> datasource.removeTag(normalizedCollection,
                    normalizedValue));
        }
...
//region Private helpers
    ...
    private String normalizeAttributeKey(final String key) throws AttributeValidationException
    {
        if (TextUtils.isEmpty(key) || !ATTR_KEY_PATTERN.matcher(key).matches()) {
            Logger.error(TAG,
                    "Invalid key. Please make sure that the key is made of letters, underscores and numbers only (a-zA-Z0-9_). It also can't be longer than 30 characters. Ignoring attribute '" + key + "'.");
