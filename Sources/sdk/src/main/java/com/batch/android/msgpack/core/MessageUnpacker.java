...
import com.batch.android.msgpack.core.MessagePack.Code;
...
import com.batch.android.msgpack.value.ValueFactory;
import com.batch.android.msgpack.value.Variable;
...
public class MessageUnpacker implements Closeable {
  ...
  private static final MessageBuffer EMPTY_BUFFER = MessageBuffer.wrap(
    new byte[0]
  );
   ...
   * @param in
   */
  protected MessageUnpacker(
    MessageBufferInput in,
    ...
    while (count > 0) {
      ...
      switch (f) {
        ...
        case MAP32:
          ...
          break;
        case NEVER_USED:
          throw new MessageNeverUsedFormatException(
            "Encountered 0xC1 \"NEVER_USED\" byte"
          );
      }
  ...
  private static MessagePackException unexpected(String expected, byte b) {
    ...
    if (format == MessageFormat.NEVER_USED) {
      return new MessageNeverUsedFormatException(
        String.format(
          Locale.US,
          "Expected %s, but encountered 0xC1 \"NEVER_USED\" byte",
          expected
        )
      );
    } else {
      ...
      String typeName =
        name.substring(0, 1) + name.substring(1).toLowerCase(Locale.US);
      return new MessageTypeException(
        String.format(
          Locale.US,
          "Expected %s, but got %s (%02x)",
          expected,
          typeName,
          b
        )
  ...
  public String unpackString() throws IOException {
    ...
    if (len > stringSizeLimit) {
      throw new MessageSizeException(
        String.format(
          "cannot unpack a String of size larger than %,d: %,d",
          stringSizeLimit,
          len
        ),
        len
      ...
      while (rawRemaining > 0) {
        ...
        if (bufferRemaining >= rawRemaining) {
          ...
          if (cr.isUnderflow() && readLen < bufferRemaining) {
            ...
            int incompleteMultiBytes = utf8MultibyteCharacterSize(
              buffer.getByte(position)
            );
            ByteBuffer multiByteBuffer = ByteBuffer.allocate(
            ...
            if (
              ...
              (
                cr.isUnderflow() &&
                multiByteBuffer.position() < multiByteBuffer.limit()
              )
            ) {
              ...
              try {
                ...
                throw new MessageFormatException(
                  "Unexpected UTF-8 multibyte sequence"
                );
              } catch (Exception ex) {
                throw new MessageFormatException(
                  "Unexpected UTF-8 multibyte sequence",
  ...
  private void handleCoderError(CoderResult cr)
    ...
    if (
      (
        cr.isMalformed() && actionOnMalformedString == CodingErrorAction.REPORT
      ) ||
      (
        cr.isUnmappable() &&
        actionOnUnmappableString == CodingErrorAction.REPORT
      )
    ) {
      cr.throwException();
    }
  ...
  private String decodeStringFastPath(int length) {
    if (
      actionOnMalformedString == CodingErrorAction.REPLACE &&
