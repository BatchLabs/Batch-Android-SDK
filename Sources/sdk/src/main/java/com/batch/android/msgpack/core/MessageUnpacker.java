        ...
        implements Closeable
{
    ...
    private static MessagePackException unexpected(String expected, byte b)
    {
        ...
        if (format == MessageFormat.NEVER_USED) {
            return new MessageNeverUsedFormatException(String.format(Locale.US,
                    "Expected %s, but encountered 0xC1 \"NEVER_USED\" byte",
                    expected));
        } else {
            String name = format.getValueType().name();
            String typeName = name.substring(0, 1) + name.substring(1).toLowerCase(Locale.US);
            return new MessageTypeException(String.format(Locale.US,
            ...
            throws IOException
    {
        ...
        switch (mf.getValueType()) {
            ...
            case EXTENSION: {
                ...
                return ValueFactory.newExtension(extHeader.getType(),
            ...
            throws IOException
    {
        ...
        if (len > stringSizeLimit) {
            throw new MessageSizeException(String.format(
            ...
            while (rawRemaining > 0) {
                ...
                if (bufferRemaining >= rawRemaining) {
                    ...
                    if (cr.isUnderflow() && readLen < bufferRemaining) {
                        ...
                        int incompleteMultiBytes = utf8MultibyteCharacterSize(buffer.getByte(
                                position));
