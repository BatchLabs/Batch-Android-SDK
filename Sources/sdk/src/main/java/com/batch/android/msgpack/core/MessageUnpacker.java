    ...
    private static MessagePackException unexpected(String expected, byte b) {
        ...
        if (format == MessageFormat.NEVER_USED) {
            return new MessageNeverUsedFormatException(
                String.format(Locale.US, "Expected %s, but encountered 0xC1 \"NEVER_USED\" byte", expected)
            );
        } else {
            ...
            return new MessageTypeException(
                String.format(Locale.US, "Expected %s, but got %s (%02x)", expected, typeName, b)
            );
        }
    }
    ...
    public String unpackString() throws IOException {
        ...
        if (len > stringSizeLimit) {
            throw new MessageSizeException(
                String.format("cannot unpack a String of size larger than %,d: %,d", stringSizeLimit, len),
                len
            );
        }
