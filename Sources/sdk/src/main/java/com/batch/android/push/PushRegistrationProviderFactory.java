...
public class PushRegistrationProviderFactory
{
    ...
    private static final String COMPONENT_SENTINEL_VALUE =
            "com.batch.android.push.PushRegistrationRegistrar";
    ...
    public PushRegistrationProvider getRegistrationProvider()
    {
        ...
        if (playServiceAvailableError == null || playServiceAvailableError != 0) {
            ...
            if (provider != null) {
                Logger.info(PushModule.TAG,
                        "Registration ID/Push Token: Using " + provider.getClass().getSimpleName());
                return provider;
            }
        }
    ...
    private boolean isLegacyPushReceiverInManifest()
    {
        try {
            ...
            Logger.error(PushModule.TAG,
                    "Could not check if legacy push receiver is in the manifest",
                    e);
            return false;
        }
    }
    ...
    private boolean isBatchGCMIidServiceAvailable()
    {
        try {
            ...
            final Intent intent = new Intent(RuntimeManagerProvider.get().getContext(),
                    BatchPushInstanceIDService.class);
            @SuppressLint("QueryPermissionsNeeded")
            List<ResolveInfo> resolveInfo = packageManager.queryIntentServices(intent,
    ...
    public PushRegistrationProvider getExternalPushRegistrationProvider()
    {
        ...
        for (String name : registrarNames) {
            try {
                ...
                PushRegistrationRegistrar registrar = (PushRegistrationRegistrar) loadedClass.getDeclaredConstructor().newInstance();
                PushRegistrationProvider registrationProvider = registrar.getPushRegistrationProvider(
                        context);
                if (registrationProvider != null) {
                    ...
                    try {
                        ...
                        if (!isExternalProviderAllowed(shortname)) {
                            Logger.internal(TAG,
                                    "Found '" + shortname + "' (" + registrationProvider.getClass().getSimpleName() + ") which is not allowed, skipping...");
                            continue;
                        }
                        ...
                        Logger.internal(TAG,
                                "Tried to use " + registrationProvider.getClass().getSimpleName() + " but not available, skipping...");
                    }
                } else {
                    Logger.internal(TAG,
                            "Registrar " + registrar.getClass().getSimpleName() + " did not return a PushRegistrationProvider, skipping...");
                }
            } catch (Throwable e) {
