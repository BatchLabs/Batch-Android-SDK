...
public class PushRegistrationProviderFactory {
  ...
  public PushRegistrationProvider getRegistrationProvider() {
      ...
      if (gcmSenderID != null) {
        ...
        if (shouldUseGoogleInstanceID) {
          if (isGCMInstanceIdAvailable()) {
            ...
            Logger.internal(
              TAG,
              "GCM Instance ID class unavailable, falling back"
            );
            canUseGCMInstanceID = false;
          }
        } else {
          Logger.internal(
            PushModule.TAG,
            "GCM Instance ID disabled by configuration"
          );
          canUseGCMInstanceID = false;
        }
    ...
    Integer playServiceAvailableError = GooglePlayServicesHelper.isFCMAvailable(
      context
    );
    if (playServiceAvailableError == null || playServiceAvailableError != 0) {
      ...
      if (provider != null) {
        Logger.info(
          PushModule.TAG,
          "Registration ID/Push Token: Using " +
    ...
    if (isFCMTokenApiAvailable()) {
      if (isSenderIdOverridden()) {
        ...
        Logger.internal(
          TAG,
          "FCM InstanceId provider is forced from the manifest."
        );
      }
    } else {
      shouldUseFCMInstanceId = true;
  ...
  private boolean isLegacyPushReceiverInManifest() {
    try {
      ...
      List<ResolveInfo> resolveInfo = packageManager.queryBroadcastReceivers(
        intent,
        matchFlag
      );
  ...
  private boolean shouldForceFirebaseIIDProvider() {
    return MetaDataUtils.getBooleanMetaData(
      context,
      MetaDataUtils.MANIFEST_FORCE_FCM_IID_KEY
    );
  }
  ...
  private boolean isSenderIdOverridden() {
    return (
      MetaDataUtils.getIntMetaData(
        context,
        MetaDataUtils.MANIFEST_SENDER_ID_KEY
      ) !=
      -1
    );
  }
