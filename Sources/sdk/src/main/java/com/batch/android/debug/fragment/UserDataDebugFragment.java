...
public class UserDataDebugFragment extends Fragment {
    ...
    Batch.User.fetchAttributes(
      ...
      new BatchAttributesFetchListener() {
        ...
        public void onSuccess(
          @NonNull Map<String, BatchUserAttribute> attributes
        ) {
          attributeAdapter.setNotifyOnChange(false);
          for (Map.Entry<String, BatchUserAttribute> entry : attributes.entrySet()) {
            attributeAdapter.add(
              entry.getKey() + ": " + formatAttribute(entry.getValue())
            );
          }
          attributeAdapter.notifyDataSetChanged();
        }
  ...
  private void loadCollections() {
    ...
    Batch.User.fetchTagCollections(
      ...
      new BatchTagCollectionsFetchListener() {
        @Override
        public void onSuccess(
          @NonNull Map<String, Set<String>> tagCollections
        ) {
  ...
  public View onCreateView(
    ...
    View view = inflater.inflate(
      R.layout.com_batchsdk_user_data_debug_fragment,
      container,
      false
    );
    customUserId =
      view.findViewById(R.id.com_batchsdk_user_data_debug_fragment_user_id);
    attributeList =
      view.findViewById(
        R.id.com_batchsdk_user_data_debug_fragment_attribute_list
      );
    collectionList =
      view.findViewById(
        R.id.com_batchsdk_user_data_debug_fragment_collection_list
      );
    return view;
  }
