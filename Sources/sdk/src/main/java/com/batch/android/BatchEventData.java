...
public class BatchEventData
{
    ...
    BatchEventData(@NonNull JSONObject fromJSON)
    {
        ...
        TreeSet<String> sortedLegacyKeys = new TreeSet<>((o1, o2) -> o1.toLowerCase(Locale.US).compareTo(
                o2.toLowerCase(Locale.US)));
    ...
    public BatchEventData put(String key, Date value)
    {
        if (enforceAttributeName(key) && enforceAttributesCount(key) && enforceDateValue(value)) {
            attributes.put(normalizeKey(key),
    ...
    JSONObject toInternalJSON() throws JSONException
    {
        ...
        for (Map.Entry<String, TypedAttribute> entry : this.attributes.entrySet()) {
            ...
            attributes.put(entry.getKey().toLowerCase(Locale.US) + "." + attribute.type.getTypeChar(),
                    attribute.value);
        }
    ...
    private boolean enforceAttributeName(String key)
    {
        if (TextUtils.isEmpty(key) || !BatchUserDataEditor.ATTR_KEY_PATTERN.matcher(key).matches()) {
            Logger.internal(TrackerModule.TAG,
                    "BatchEventData: Invalid key. Please make sure that the key is made of letters, underscores and numbers only (a-zA-Z0-9_). It also can't be longer than 30 characters. Ignoring value '" + key + "'.");
            return false;
        }
