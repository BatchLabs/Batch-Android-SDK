...
public class Parser {
  ...
  public Parser(
    @NonNull ImportFileProvider importFileProvider,
    String cssString
  ) {
  ...
  private void fillImports() {
    ...
    while (matcher.find()) {
      final String importContent = importFileProvider.getContent(
        matcher.group(1)
      );
  ...
  private void consumeToken(String token) {
    currentToken =
      (shouldMergePreviousToken ? currentToken : "") +
      (token != null ? token.trim() : "");
    shouldMergePreviousToken = false;
  }

  private void consumeSpecialToken(char tokenFirstChar)
  ...
  public void switchOutOfRulesetState() throws CSSParsingException {
    ...
    if (
      substate != Substate.PROPERTY_NAME &&
      (state == State.MEDIA_QUERY && substate != Substate.SELECTOR)
    ) {
      throwGenericParsingException();
    }
