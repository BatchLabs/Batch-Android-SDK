...
{
    public final static int MAIN_DEBUG_FRAGMENT = 0;
    public final static int IDENTIFIER_DEBUG_FRAGMENT = 1;
    public final static int USER_DATA_DEBUG_FRAGMENT = 2;
    public final static int LOCAL_CAMPAIGNS_DEBUG_FRAGMENT = 3;
    public final static int LOCAL_CAMPAIGN_DEBUG_FRAGMENT = 4;
    ...
    private void switchFragment(int newIndex, boolean first, String campaignToken)
    {
        if (newIndex >= 0 && newIndex < fragments.length) {
            if (fragments[newIndex] == null) {
                switch (newIndex) {
                    ...
                    case LOCAL_CAMPAIGNS_DEBUG_FRAGMENT:
                        fragments[newIndex] = LocalCampaignsDebugFragment.newInstance(
                                CampaignManagerProvider.get());
                        break;
                    case LOCAL_CAMPAIGN_DEBUG_FRAGMENT:
                        fragments[newIndex] = LocalCampaignDebugFragment.newInstance(campaignToken,
                                CampaignManagerProvider.get());
                        break;
                }
            }
            ...
            if (first) {
                fragmentTransaction.replace(R.id.com_batchsdk_debug_fragment_container,
                        fragments[newIndex]).commitNow();
            } else {
                fragmentTransaction.replace(R.id.com_batchsdk_debug_fragment_container,
                        fragments[newIndex]).addToBackStack(null).commit();
            }
        }
    }
