 ...
 *
 */
public class BatchDebugActivity
  extends FragmentActivity
  implements OnMenuSelectedListener {
  ...
  private void switchFragment(
    ...
    if (newIndex >= 0 && newIndex < fragments.length) {
      if (fragments[newIndex] == null) {
        switch (newIndex) {
          ...
          case LOCAL_CAMPAIGNS_DEBUG_FRAGMENT:
            fragments[newIndex] =
              LocalCampaignsDebugFragment.newInstance(
                CampaignManagerProvider.get()
              );
            break;
          case LOCAL_CAMPAIGN_DEBUG_FRAGMENT:
            fragments[newIndex] =
              LocalCampaignDebugFragment.newInstance(
                campaignToken,
                CampaignManagerProvider.get()
              );
            break;
      ...
      if (first) {
        fragmentTransaction
          .replace(
            R.id.com_batchsdk_debug_fragment_container,
            fragments[newIndex]
          )
          .commitNow();
      } else {
