...
public final class EventContext implements EvaluationContext
{
    ...
    public EventContext(String name, @Nullable String label, @Nullable BatchEventData data)
    {
        eventName = name;
        if (label != null) {
            eventLabel = label;
    ...
    {
        if (isPublicEvent && data != null) {
            return new PrimitiveValue(BatchEventDataPrivateHelper.getConvertedFromLegacyAPIFromEvent(
                    data));
        }
    ...
    private Value dataForRawVariableName(String variableName)
    {
        if (isPublicEvent && data != null) {
            ...
            String wantedAttributeName = extractAttributeFromVariableName(variableName);
    ...
    private String extractAttributeFromVariableName(String variableName)
    {
        ...
        if (variableName.endsWith("']") && variableName.length() > 10) {
            String extractedName = variableName.substring(8, 8 + variableName.length() - 10);
