...
public class SessionManager implements ComponentCallbacks2, Application.ActivityLifecycleCallbacks
{
    ...
    {
        if (sessionActive && backgroundSessionExpirationUptime != null && SystemClock.elapsedRealtime() >= backgroundSessionExpirationUptime) {
            sessionActive = false;
    ...
    synchronized void startNewSessionIfNeeded(Context c)
    {
        ...
        if (!sessionActive) {
            ...
            LocalBroadcastManagerProvider.get(c).sendBroadcast(new Intent(
                    INTENT_NEW_SESSION));
        }
    }
