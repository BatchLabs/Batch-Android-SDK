...
public class SessionManager
   ...
   */
  public static final String INTENT_NEW_SESSION =
    Parameters.LIBRARY_BUNDLE + ".runtime.session.new";
  ...
  synchronized void startNewSessionIfNeeded(Context c) {
    ...
    if (!sessionActive) {
      ...
      Logger.internal(
        TAG,
        "Starting a new session, id: '" + sessionIdentifier + "'"
      );
      LocalBroadcastManagerProvider
        .get(c)
        .sendBroadcast(new Intent(INTENT_NEW_SESSION));
    }
  }
  ...
  public void onTrimMemory(int level) {
    if (level >= ComponentCallbacks2.TRIM_MEMORY_UI_HIDDEN) {
      backgroundSessionExpirationUptime =
        getUptime() + BACKGROUNDED_SESSION_EXPIRATION_SEC * 1000;
    }
  }
  ...
  public void onActivityDestroyed(Activity activity) {
    ...
    if (areAllActivitiesDestroyed()) {
      ...
      Logger.internal(
        TAG,
        "Finishing session since the last activity has been destroyed"
      );
    }
  }
}
