...
public class Parser {
  ...
  private static final char TOKEN_DELIMITER_VARIABLE = '`';
  private static final char TOKEN_DELIMITER_STRING = '"';
  private static final char TOKEN_DELIMITER_LIST_START = '(';
  private static final char TOKEN_DELIMITER_STRING_ARRAY_START = '[';
  ...
  public Value parse() {
    ...
    return errorWithPositionAndMessage(
      "Expected " + TOKEN_DELIMITER_LIST_START
    );
  }
          ...
          if (tmpVal == null) {
            tmpVal =
              errorWithMessage(
                ...
                "': It is not an operator, and it could not be converted to a number"
              );
          }
    ...
    while (!endReached) {
      ...
      if (c == ']') {
        ...
        if (stringValue instanceof ErrorValue) {
          ...
          stringValue instanceof PrimitiveValue &&
        ...
        return errorWithPositionAndMessage(
          "Unexpected character '" + c + "' in string array, expected \" or ]"
        );
      }
    }
  ...
  private Value parseVariable() {
    ...
    while (!endReached) {
      ...
      if (c == TOKEN_DELIMITER_VARIABLE) {
        String trimmed = stringByTrimmingWhiteSpaceAndNewline(
  ...
  private ErrorValue errorWithPositionAndMessage(String message) {
    return new ErrorValue(
      ErrorValue.Type.Parser,
      "At position" + _pos + message
    );
  }
