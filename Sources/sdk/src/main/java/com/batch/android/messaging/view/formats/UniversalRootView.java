...
public class UniversalRootView
  extends PercentRelativeLayout
  implements SeparatedFlexboxLayout.SeparatorStyleProvider {
  ...
  private void createViews() {
    ...
    if (
      waitForHeroImage || heroDownloadResult != null || message.videoURL != null
    ) {
      setupHeroLayout();
    }
  ...
  private void setupContentLayout() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      contentLayout.setImportantForAccessibility(
    ...
    bodyScrollWrapper.setLayoutParams(
      StyleHelper.getFlexLayoutParams(context, bodyScrollWrapperLp, bodyTvRules)
    );
    ...
    final ViewGroup targetCTALayout = ctasLayout != null
      ? ctasLayout
      : contentLayout;
    if (
      message.stackCTAsHorizontally != null && message.stackCTAsHorizontally
    ) {
    ...
    for (Pair<View, DOMNode> ctaViewPair : ctaViews) {
      ...
      if (
        ctasLayout != null &&
        message.stretchCTAsHorizontally != null &&
  ...
  private void setupHeroLayout() {
    ...
    if (message.videoURL != null) {
      ...
      if (!TextUtils.isEmpty(message.heroDescription)) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
          heroImageView.setImportantForAccessibility(
            IMPORTANT_FOR_ACCESSIBILITY_YES
          );
        }
      }
      ...
      if (heroDownloadResult == null) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          if (darkProgressBar) {
            heroLoader.setIndeterminateTintList(
    ...
    for (Pair<View, DOMNode> viewPair : views) {
      ...
      view.setLayoutParams(
        StyleHelper.getFrameLayoutParams(context, null, rules)
      );
      if (view instanceof Styleable) {
        ((Styleable) view).applyStyleRules(rules);
      } else {
      ...
      if (!landscape) {
        ...
        heroLP.getPercentLayoutInfo().heightPercent =
          message.heroSplitRatio != null
            ? message.heroSplitRatio.floatValue()
            : 0.4f;
    ...
    if (contentLayout != null) {
      contentLayout.setPadding(
        ...
        shouldApplyWindowInsetToContent()
          ? originalContentPaddingTop + topInset
          : originalContentPaddingTop,
  ...
  private boolean shouldApplyWindowInsetToContent() {
    ...
    return (
      heroLayout == null ||
      (message != null && message.flipHeroVertical == Boolean.TRUE) ||
      landscape
    );
  }
