 ...
 *
 */
public class AsyncImageDownloadTask
  extends AsyncTask<String, Void, AsyncImageDownloadTask.Result> {
  ...
  protected Result doInBackground(String... params) {
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Error while reading the image URL (" + params[0] + ")",
        e
      );
      return null;
    }
    ...
    try {
      ...
      connection.setReadTimeout(
        (int) (MessagingModule.DEFAULT_IMAGE_DOWNLOAD_TIMEOUT * 1000)
      );
      ...
      if (statusCode >= 200 && statusCode < 300) {
        ...
        if (GifHelper.isPotentiallyAGif(fris.getFirstBytes())) {
          return new GIFResult(
            params[0],
            ByteArrayHelper.fromInputStream(fris)
          );
        } else {
        ...
        Logger.internal(
          TAG,
          "Server returned an invalid status code (" +
          statusCode +
          ") for URL (" +
          params[0] +
          ")"
      ...
      ClassCastException | NoSuchAlgorithmException | KeyManagementException e2
    ) {
    ...
    if (listener != null) {
      if (result != null) {
        ...
        listener.onImageDownloadError(
          lastError != null ? lastError : MessagingError.UNKNOWN
        );
      }
    }
  }
