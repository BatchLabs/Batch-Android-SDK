...
public final class URLBuilder {
   ...
   */
  public URLBuilder(String urlPattern, CryptorMode mode, String[] parameters)
    throws MalformedURLException {
   ...
   * @throws MalformedURLException
   */
  private void parseURL(String urlPattern, String[] parameters)
    throws MalformedURLException {
    URL url = null;
    ...
    try {
      ...
      if (parameters != null && parameters.length > 0) {
        ...
        for (String parameter : parameters) {
          ...
          encodedParameters.add(
            URLEncoder.encode(parameter, ByteArrayHelper.UTF_8)
          );
        }
      ...
      throw new MalformedURLException(
        "Error while initializing URL with pattern : " + urlString
      );
    }
        ...
        case VALUE:
          {
            for (String key : sorter.getKeysOrdered(getParameters)) {
              addParameter(
                builder,
                key,
                cryptor.encrypt(getParameters.get(key))
              );
            }
