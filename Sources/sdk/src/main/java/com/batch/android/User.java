...
final class User {
  ...
  public void setLanguage(@Nullable String language) {
    if (language != null) {
      ParametersProvider
        .get(context)
        .set(ParameterKeys.USER_PROFILE_LANGUAGE_KEY, language, true);
    } else {
      ParametersProvider
        .get(context)
        .remove(ParameterKeys.USER_PROFILE_LANGUAGE_KEY);
    }
  }
  ...
  public String getLanguage() {
    return ParametersProvider
      .get(context)
      .get(ParameterKeys.USER_PROFILE_LANGUAGE_KEY);
  }
  ...
  public void setRegion(@Nullable String region) {
    if (region != null) {
      ParametersProvider
        .get(context)
        .set(ParameterKeys.USER_PROFILE_REGION_KEY, region, true);
    } else {
      ParametersProvider
        .get(context)
        .remove(ParameterKeys.USER_PROFILE_REGION_KEY);
    }
  }
  ...
  public String getRegion() {
    return ParametersProvider
      .get(context)
      .get(ParameterKeys.USER_PROFILE_REGION_KEY);
  }
  ...
  public void setCustomID(@Nullable String customID) {
    if (customID != null) {
      ParametersProvider
        .get(context)
        .set(ParameterKeys.CUSTOM_ID, customID, true);
    } else {
  ...
  public long getVersion() {
    String version = ParametersProvider
      .get(context)
      .get(ParameterKeys.USER_DATA_VERSION);
    if (version == null) {
      return 1;
    }
  ...
  private synchronized long incrementVersion() {
    ...
    ParametersProvider
      .get(context)
      .set(ParameterKeys.USER_DATA_VERSION, Long.toString(newVersion), true);
    return newVersion;
  }
  ...
  public void sendChangeEvent() {
    try {
      ...
      Logger.internal(
        UserModule.TAG,
        "Could not track " + InternalEvents.PROFILE_CHANGED,
        e
      );
    }
  }
