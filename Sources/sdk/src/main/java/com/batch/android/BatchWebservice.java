...
public abstract class BatchWebservice extends Webservice {
  ...
  protected PostDataProvider<JSONObject> getPostDataProvider() {
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Error while adding module parameters into parameters",
        e
      );
    }
    ...
    if (
      !TextUtils.isEmpty(advancedIdsParameterString) &&
    ...
    for (String parameter : parameters) {
      try {
        if (SystemParameterShortName.INSTALL_ID.shortName.equals(parameter)) {
          ...
          SystemParameterShortName.DEVICE_INSTALL_DATE.shortName.equals(
            parameter
          )
        ) {
          Install install = Batch.getInstall();
          ...
          SystemParameterShortName.SERVER_ID.shortName.equals(parameter)
        ) {
          String val = ParametersProvider
            .get(applicationContext)
            .get(ParameterKeys.SERVER_ID_KEY);
          if (val != null) {
            ids.put(parameter, val);
          }
        } else if (
          SystemParameterShortName.SESSION_ID.shortName.equals(parameter)
        ) {
          ...
          SystemParameterShortName.CUSTOM_USER_ID.shortName.equals(parameter)
        ) {
          ...
          SystemParameterShortName.ADVERTISING_ID.shortName.equals(parameter)
        ) {
          if (Batch.shouldUseAdvertisingID()) {
            AdvertisingID advertisingID = Batch.getAdvertisingID();
            if (advertisingID != null) {
              boolean isIdfaAvailable =
                advertisingID.isReady() && advertisingID.isNotNull();
              if (isIdfaAvailable) {
          ...
          SystemParameterShortName.ADVERTISING_ID_OPTIN.shortName.equals(
            parameter
          )
        ) {
          AdvertisingID advertisingID = Batch.getAdvertisingID();
          ...
          SystemParameterShortName.BRIDGE_VERSION.shortName.equals(parameter)
        ) {
          ...
          SystemParameterShortName.PLUGIN_VERSION.shortName.equals(parameter)
        ) {
          ...
          String val = SystemParameterHelper.getValue(
            parameter,
            applicationContext
          );
          if (val != null) {
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Error while adding ids object to global post params",
        e
      );
    }
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Error while adding retry count data to global post params",
        e
      );
    }
     ...
     */
    try {
      Map<String, Metric> metrics = WebserviceMetricsProvider
        .get()
        .getMetrics();
      ...
      if (parameterKey != null && parameterKey.length() > 0) {
        ...
        for (String string : list) {
          ...
          if (data == null || data.length() == 0) {
            Logger.internal(
              TAG,
              "Unable to find parameter value for key " + string
            );
            continue;
          }
  ...
  protected void handleParameters(JSONObject body) {
    ...
    try {
      if (body.has("parameters") && !body.isNull("parameters")) {
        ...
        for (int i = 0; i < parametersArray.length(); i++) {
          ...
          try {
            ...
            boolean save = parameterObject.has("s") &&
              !parameterObject.isNull("s")
      ...
      Logger.internal(
        TAG,
        "Error while reading parameters into WS response",
        e
      );
    }
  }
  ...
  protected void handleServerID(JSONObject body) {
    ...
    try {
      if (body.has("i") && !body.isNull("i")) {
        ParametersProvider
          .get(applicationContext)
          .set(ParameterKeys.SERVER_ID_KEY, body.getString("i"), true);
      }
    } catch (Exception e) {
  ...
  private static String generateUserAgent(Context applicationContext) {
    try {
      ...
      String bundleName = SystemParameterHelper.getBundleName(
        applicationContext
      );
      String versionApp = SystemParameterHelper.getAppVersion(
        applicationContext
      );
      ...
      String pluginUA = String
        .format("%s %s", pluginVersion, bridgeVersion)
        .trim();
