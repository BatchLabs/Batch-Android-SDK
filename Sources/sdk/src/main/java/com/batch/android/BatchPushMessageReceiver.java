    ...
    if (msgType == null || "gcm".equalsIgnoreCase(msgType)) {
      ...
      if (
        ...
        try {
          ...
          jobExtras.putBundle(
            BatchPushJobService.JOB_EXTRA_PUSH_DATA_KEY,
            intentExtras
          );
          ...
          if (scheduler.schedule(job) == JobScheduler.RESULT_FAILURE) {
            Logger.internal(
              TAG,
              "Failed to schedule the push notification presenter job"
            );
          } else {
            Logger.internal(
              TAG,
              "Successfully scheduled the push notification presenter job"
            );
          }
          ...
          Logger.internal(
            TAG,
            "Could schedule Batch push presentation job",
            e1
          );
        }
      } else {
        ...
        ComponentName comp = new ComponentName(
          ...
          BatchPushService.class.getName()
        );
