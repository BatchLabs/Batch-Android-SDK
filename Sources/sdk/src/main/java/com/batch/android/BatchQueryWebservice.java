...
// ------------------------------------------->
     ...
     */
    protected BatchQueryWebservice(Context context,
                                   RequestType type,
...
// ------------------------------------------->
    ...
    private void parseQueries(JSONObject jsonResponse) throws JSONException, IllegalStateException
    {
        ...
        if (responses.length() != queries.size()) {
            throw new IllegalStateException("Number of queries and responses mismatch(" + queries.size() + " queries / " + responses.length() + " responses)");
        }
    ...
    protected <T extends Response> T getResponseFor(Class<T> clazz,
                                                    QueryType type) throws ClassCastException
    {
        if (responses == null) {
