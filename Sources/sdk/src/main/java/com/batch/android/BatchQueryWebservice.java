...
abstract class BatchQueryWebservice extends BatchWebservice {
   ...
   */
  protected BatchQueryWebservice(
    Context context,
    RequestType type,
   ...
   */
  private void parseQueries(JSONObject jsonResponse)
    throws JSONException, IllegalStateException {
      ...
      switch (query.getType()) {
        ...
        case ATTRIBUTES:
          responseDeserializer =
            new AttributesSendResponseDeserializer(response);
          break;
        case ATTRIBUTES_CHECK:
          responseDeserializer =
            new AttributesCheckResponseDeserializer(response);
          break;
        case LOCAL_CAMPAIGNS:
          responseDeserializer =
            new LocalCampaignsResponseDeserializer(response);
          break;
      }
      // Build the response
  ...
  protected <T extends Response> T getResponseFor(
    Class<T> clazz,
    QueryType type
  ) throws ClassCastException {
    if (responses == null) {
      throw new IllegalStateException(
        "You forgot to call parseResponse method"
      );
    }
