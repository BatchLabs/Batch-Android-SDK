...
public class ActionModule extends BatchModule
{
    ...
    public void registerAction(@NonNull UserAction userAction)
    {
        ...
        if (identifier.startsWith(RESERVED_ACTION_IDENTIFIER_PREFIX)) {
            throw new IllegalArgumentException("The action identifier ('" + identifier
                    + "') is using a reserved prefix (" + RESERVED_ACTION_IDENTIFIER_PREFIX + ")");
        }

        if (registeredActions.put(identifier.toLowerCase(Locale.US), userAction) != null) {
            Logger.warning(TAG,
                    "Action '" + identifier + "' was already registered, and will be replaced.");
        }
    }
     ...
     */
    public boolean performUserAction(@Nullable Context context,
                                     @NonNull String identifier,
                                     @Nullable JSONObject args)
    {
        if (TextUtils.isEmpty(identifier) || identifier.toLowerCase(Locale.US).startsWith(
     ...
     * @return true if an action was registered for this identifier and performed, false otherwise.
     */
    public boolean performAction(@Nullable Context context,
                                 @NonNull String identifier,
                                 ...
                                 @Nullable UserActionSource source)
    {
        ...
        if (userAction != null) {
            userAction.getRunnable().performAction(context,
                    userAction.getIdentifier(),
                    args,
                    source);
            return true;
        }
        ...
        Logger.error(TAG,
                "Batch attempted to perform the action named '" + identifier + "' but couldn't find any action registered to handle this.");
    ...
    {
        registeredActions.put(GroupActionRunnable.IDENTIFIER,
                new UserAction(GroupActionRunnable.IDENTIFIER,
                        new GroupActionRunnable(this)));

        registeredActions.put(DeeplinkActionRunnable.IDENTIFIER,
                new UserAction(DeeplinkActionRunnable.IDENTIFIER,
                        new DeeplinkActionRunnable(this)));

        registeredActions.put(LocalCampaignsRefreshActionRunnable.IDENTIFIER,
                new UserAction(LocalCampaignsRefreshActionRunnable.IDENTIFIER,
                        new LocalCampaignsRefreshActionRunnable()));

        registeredActions.put(UserDataBuiltinActionRunnable.IDENTIFIER,
                new UserAction(UserDataBuiltinActionRunnable.IDENTIFIER,
                        new UserDataBuiltinActionRunnable()));

        registeredActions.put(UserEventBuiltinActionRunnable.IDENTIFIER,
                new UserAction(UserEventBuiltinActionRunnable.IDENTIFIER,
                        new UserEventBuiltinActionRunnable()));

        registeredActions.put(ClipboardActionRunnable.IDENTIFIER,
                new UserAction(ClipboardActionRunnable.IDENTIFIER,
                        new ClipboardActionRunnable()));

        registeredActions.put(RatingActionRunnable.IDENTIFIER,
                new UserAction(RatingActionRunnable.IDENTIFIER,
                        new RatingActionRunnable()));
    }

    public int getDrawableIdForNameOrAlias(@NonNull Context context,
                                           @Nullable String drawableName)
    {
        ...
        if (resId == 0) {
            ...
            resId = context.getResources().getIdentifier(drawableName,
                    "drawable",
