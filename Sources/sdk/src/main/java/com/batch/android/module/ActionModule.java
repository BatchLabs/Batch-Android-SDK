...
public class ActionModule extends BatchModule {
  ...
  public void registerAction(@NonNull UserAction userAction) {
    ...
    if (
      registeredActions.put(identifier.toLowerCase(Locale.US), userAction) !=
      null
    ) {
      Logger.warning(
        TAG,
        "Action '" +
        identifier +
        "' was already registered, and will be replaced."
      );
    }
  }
  ...
  public void addDrawableAlias(
    ...
    if (drawableResID == 0) {
      throw new IllegalArgumentException(
        "drawable ressource ID must be different than 0"
      );
    }
   ...
   */
  public boolean performUserAction(
    ...
    if (
      ...
      identifier
        ...
        .startsWith(RESERVED_ACTION_IDENTIFIER_PREFIX)
    ) {
      Logger.internal(
        TAG,
        "Identifier is null, empty, or starts with " +
        RESERVED_ACTION_IDENTIFIER_PREFIX +
        " . Aborting."
      );
      return false;
  ...
  public boolean performAction(
    ...
    final UserAction userAction = registeredActions.get(
      identifier.toLowerCase(Locale.US)
    );

    if (userAction != null) {
      userAction
        .getRunnable()
        .performAction(context, userAction.getIdentifier(), args, source);
      return true;
    }
   ...
   */
  public void setDeeplinkInterceptor(
  ...
  public int getDrawableIdForNameOrAlias(
