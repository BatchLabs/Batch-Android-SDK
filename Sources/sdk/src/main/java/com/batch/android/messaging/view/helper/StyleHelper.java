...
public class StyleHelper {
    ...
    try {
      if (css == null) {
        throw new IllegalArgumentException(
          "Message's style string cannot be null"
        );
      }
   ...
   */
  public static void applyCommonRules(
    View targetView,
    ...
    for (Map.Entry<String, String> rule : rules.entrySet()) {
      if ("background".equalsIgnoreCase(rule.getKey())) {
        ...
        if (value.startsWith("#")) {
          ...
          value.startsWith("linear-gradient(") && value.endsWith(")")
        ) {
          ...
          switch (angle.intValue()) {
            ...
            case 0:
              gradientOrientation =
                PositionableGradientDrawable.Orientation.BOTTOM_TOP;
              break;
            case 90:
              gradientOrientation =
                PositionableGradientDrawable.Orientation.LEFT_RIGHT;
              break;
            case 180:
              gradientOrientation =
                PositionableGradientDrawable.Orientation.TOP_BOTTOM;
              break;
            case -90:
            case 270:
              gradientOrientation =
                PositionableGradientDrawable.Orientation.RIGHT_LEFT;
              break;
          }
          ...
          for (int i = 1; i < gradientArguments.length; i++) {
            ...
            // "%" will be stripped
            String[] components =
              gradientArguments[i].replace("%", "").trim().split("\\s+", 2);
          ...
          backgroundGradient =
            new PositionableGradientDrawable(
              gradientOrientation,
              colors,
              positionsArray
            );
    ...
    if (!(targetView instanceof TextureView)) {
      ...
      if (
        ...
        // Stroke
        if (borderWidth != null) {
          Integer color = backgroundColor != null
            ? backgroundColor
            : borderColor;
          if (color != null) {
  ...
  public static PercentRelativeLayout.LayoutParams getRelativeLayoutParams(
    ...
    for (Map.Entry<String, String> rule : rules.entrySet()) {
      if ("margin-left".equalsIgnoreCase(rule.getKey())) {
        ...
        "width".equalsIgnoreCase(rule.getKey()) ||
        "size".equalsIgnoreCase(rule.getKey())
      ) {
        ...
        if ("left".equals(rule.getValue())) {
          ...
          "center".equals(rule.getValue()) || "auto".equals(rule.getValue())
        ) {
          lp.addRule(RelativeLayout.CENTER_HORIZONTAL);
          hAlignSet = true;
        }
      } else if (
        "align-v".equalsIgnoreCase(rule.getKey()) ||
        "vertical-align".equalsIgnoreCase(rule.getKey())
      ) {
   ...
   */
  private static Drawable getPressableGradientDrawable(
    PositionableGradientDrawable baseDrawable
  ) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      LayerDrawable ld = new LayerDrawable(
        new Drawable[] { baseDrawable, darkDrawable }
      );
