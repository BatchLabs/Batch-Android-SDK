...
public class RoundedDrawable extends Drawable {
  ...
  private final boolean[] mCornersRounded = new boolean[] {
    true,
    true,
    true,
    true,
  ...
  private ColorStateList mBorderColor = ColorStateList.valueOf(
  ...
  public static Drawable fromDrawable(Drawable drawable) {
    if (drawable != null) {
      if (drawable instanceof Animatable) {
        // Can't easily round an animatable, so don't implement this (for now?)
        return drawable;
  ...
  private void updateShaderMatrix() {
    ...
    switch (mScaleType) {
      ...
      case CENTER_INSIDE:
        ...
        if (
          mBitmapWidth <= mBounds.width() && mBitmapHeight <= mBounds.height()
        ) {
          scale = 1.0f;
        } else {
          scale =
            Math.min(
              mBounds.width() / (float) mBitmapWidth,
  ...
  public void draw(@NonNull Canvas canvas) {
    if (mRebuildShader) {
      ...
      if (
        mTileModeX == Shader.TileMode.CLAMP &&
