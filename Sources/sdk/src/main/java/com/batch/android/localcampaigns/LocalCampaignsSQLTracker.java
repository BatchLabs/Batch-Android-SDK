...
public final class LocalCampaignsSQLTracker implements ViewTracker
{
     ...
     */
    @Override
    public ViewTracker.CountedViewEvent trackViewEvent(@NonNull String campaignID) throws ViewTrackerUnavailableException
    {
        ...
        database.execSQL("INSERT INTO " + LocalCampaignEntry.TABLE_NAME + " ("
    ...
    @NonNull
    @Override
    public ViewTracker.CountedViewEvent getViewEvent(@NonNull String campaignID) throws ViewTrackerUnavailableException
    {
        ensureWritableDatabase();
        ...
        Cursor countCursor = database.rawQuery(
                "SELECT " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_COUNT + ", "
                        + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_LAST_OCCURRENCE
                        + " FROM " + LocalCampaignEntry.TABLE_NAME
                        + " WHERE " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_ID + " = ?"
    ...
    @Override
    @NonNull
    public Map<String, Integer> getViewCounts(@NonNull List<String> campaignsIds) throws ViewTrackerUnavailableException
    {
        ensureWritableDatabase();
        ...
        if (!campaignsIds.isEmpty()) {
            ...
            Cursor countCursor = database.rawQuery(
                    "SELECT " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_ID
                            + "," + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_COUNT
                            + " FROM " + LocalCampaignEntry.TABLE_NAME
                            + " WHERE " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_ID
                            + " IN (" + idsSelectArgsBuilder.toString() + ")"
    ...
    public long campaignLastOccurrence(@NonNull String campaignID) throws ViewTrackerUnavailableException
    {
        ...
        Cursor countCursor = database.rawQuery(
                "SELECT " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_LAST_OCCURRENCE
                        + " FROM " + LocalCampaignEntry.TABLE_NAME
                        + " WHERE " + LocalCampaignEntry.COLUMN_NAME_CAMPAIGN_ID + " = ?"
                , new String[]{campaignID});
