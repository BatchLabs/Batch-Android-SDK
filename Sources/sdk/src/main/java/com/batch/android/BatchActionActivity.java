...
public class BatchActionActivity extends Activity
{
    ...
    @NonNull
    private TaskStackBuilder addPayloadToTaskStackBuilder(@NonNull TaskStackBuilder stackBuilder,
     ...
     */
    private void launchDeeplink(final Intent activityIntent,
                                final String deeplink) throws DeeplinkInterceptorRuntimeException
    {
        ...
        if (interceptor != null) {
            try {
                ...
                Logger.error(TAG,
                        "Error when trying to open deeplink from interceptor. Using fallback intent.",
                        e);
        ...
        if (activityIntent != null) {
            ...
            if (!TextUtils.isEmpty(deeplink)) {
                try {
                    ...
                    Logger.error(TAG,
                            "Could not open deeplink: no activity found to handle Intent. Is it valid and your manifest well-formed? URL: " + deeplink);
                } catch (Exception e) {
                    Logger.error(TAG, "Error while trying to open a deeplink",
                            e);
                }
            } else {
