...
public class BannerView
  ...
  implements
    SeparatedFlexboxLayout.SeparatorStyleProvider,
  ...
  public BannerView(
    ...
    this.style =
      cachedStyle != null ? cachedStyle : StyleHelper.parseStyle(message.css);
  ...
  private SeparatedFlexboxLayout makeContentLayout(
    Map<String, String> contentStyleRules
  ) {
    ...
    final SeparatedFlexboxLayout layout = new SeparatedFlexboxLayout(
      context,
      "cnt",
      this
    );
    ...
    bodyLp.flexShrink = 0;
    bodyLp.minHeight =
      StyleHelper.dpToPixels(resources, (float) BODY_MIN_HEIGHT_DP);
    bodyLp.maxHeight =
      StyleHelper.dpToPixels(resources, (float) BODY_MAX_HEIGHT_DP);
  ...
  private SeparatedFlexboxLayout makeCTALayout(Map<String, String> rules) {
    ...
    for (int i = 0; i < ctas.size(); i++) {
      ...
      final String btnDirectionalClass =
        "btn-" +
        (
          message.ctaDirection == BannerMessage.CTADirection.HORIZONTAL
            ? "h"
            : "v"
        );
  ...
  private void addImage() {
    if (message.imageURL != null) {
      ...
      if (imageCache == null || imageCache.get(message.imageURL) == null) {
        ...
        final AsyncImageDownloadTask imageDownloadTask = new AsyncImageDownloadTask(
          this
        );
        imageDownloadTask.executeOnExecutor(
  ...
  private void addCloseButton() {
    ...
    if (
      message.showCloseButton || (!canAutoClose() && message.autoCloseDelay > 0)
    ) {
      final CloseButton closeButton = new CloseButton(context);
      final Map<String, String> closeButtonRules = getRulesForView(
        new DOMNode("close")
      );
  ...
  private void addCountdownView() {
    if (canAutoClose() && message.autoCloseDelay > 0) {
      final Map<String, String> countdownRules = getRulesForView(
        new DOMNode("countdown")
      );
  ...
  public VerticalEdge getPinnedVerticalEdge() {
    return pinnedVerticalEdge != null
      ? pinnedVerticalEdge
      : VerticalEdge.BOTTOM;
  }
  ...
  @Override
  public Map<String, String> getRulesForSeparator(
    SeparatedFlexboxLayout layout,
    String separatorID
  ) {
    return getRulesForView(
      new DOMNode(
        separatorID,
        layout.getSeparatorPrefix() +
        "-" +
        (layout.isHorizontal() ? "h-sep" : "sep")
      )
    );
  }
  ...
  private boolean mustWaitTapDelay() {
    return (
      SystemClock.uptimeMillis() < uptimeWhenShown + message.globalTapDelay
    );
  }
