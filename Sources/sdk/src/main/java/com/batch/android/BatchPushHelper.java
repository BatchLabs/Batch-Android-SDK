...
public class BatchPushHelper {
    ...
    public static synchronized void markPushAsShown(Context context, String pushId) {
        ...
        if (
            !ObjectUserPreferencesStorageProvider.get(context).persist(ALREADY_SHOWN_IDS_STORAGE_KEY, alreadyShownIds)
        ) {
    ...
    private static FixedSizeArrayList<String> getShownPushIds(Context context) {
        ...
        try {
            Object storedIds = ObjectUserPreferencesStorageProvider.get(context).get(ALREADY_SHOWN_IDS_STORAGE_KEY);
            if (storedIds != null) {
