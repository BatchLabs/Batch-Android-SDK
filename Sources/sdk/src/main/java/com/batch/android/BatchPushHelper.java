...
public class BatchPushHelper {
  ...
  private static final String ALREADY_SHOWN_IDS_STORAGE_KEY =
    "push_already_shown";
  ...
  private static final ArrayList<String> beingShownIds = new ArrayList<>(
    ALREADY_SHOWN_IDS_SIZE
  );

  /**
   * Internal method
   */
  public static synchronized boolean isPushValid(
    Context context,
    ...
    if (pushId != null && beingShownIds.contains(pushId)) {
      Logger.warning(
        PushModule.TAG,
        "Already shown notification[" + pushId + "], aborting"
      );
      return false;
    } else if (pushId != null) {
   ...
   * Internal method
   */
  public static synchronized void markPushAsShown(
    Context context,
    String pushId
  ) {
    if (pushId == null) {
    ...
    if (
      !ObjectUserPreferencesStorageProvider
        .get(context)
        .persist(ALREADY_SHOWN_IDS_STORAGE_KEY, alreadyShownIds)
    ) {
      Logger.internal(
        PushModule.TAG,
        "Error while saving already shown push ids"
      );
    }
   ...
   */
  @Nullable
  public static Bundle firebaseMessageToReceiverBundle(
  ...
  private static FixedSizeArrayList<String> getShownPushIds(Context context) {
    ...
    try {
      Object storedIds = ObjectUserPreferencesStorageProvider
        .get(context)
        .get(ALREADY_SHOWN_IDS_STORAGE_KEY);
      if (storedIds != null) {
   ...
   * @return
   */
  private static boolean installIDMatchesCurrent(
    Context context,
    String installId
  ) {
