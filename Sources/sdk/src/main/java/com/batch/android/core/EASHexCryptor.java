...
final class EASHexCryptor extends EASCryptor {
  ...
  public byte[] encrypt(byte[] tocrypt) {
    try {
      return ByteArrayHelper.getUTF8Bytes(
        ByteArrayHelper.bytesToHex(super.encrypt(tocrypt))
      );
    } catch (Exception e) {
  ...
  public String encrypt(String tocrypt) {
    try {
      return ByteArrayHelper.getUTF8String(
        encrypt(ByteArrayHelper.getUTF8Bytes(tocrypt))
      );
    } catch (Exception e) {
  ...
  public byte[] decrypt(byte[] crypted) {
    try {
      return super.decrypt(
        ByteArrayHelper.hexToBytes(ByteArrayHelper.getUTF8String(crypted))
      );
    } catch (Exception e) {
  ...
  public String decrypt(String string) {
    try {
      return ByteArrayHelper.getUTF8String(
        decrypt(ByteArrayHelper.getUTF8Bytes(string))
      );
    } catch (Exception e) {
