...
public class TextView extends android.widget.TextView implements Styleable {
  ...
  protected static void applyStyleRules(
    android.widget.TextView targetTextView,
    ...
    for (Map.Entry<String, String> rule : rules.entrySet()) {
      if ("color".equalsIgnoreCase(rule.getKey())) {
        try {
          ...
          Logger.internal(
            TAG,
            "Unparsable text color (" + rule.getValue() + ")",
            e
          );
        }
        ...
        "balanced".equalsIgnoreCase(rule.getKey()) &&
    ...
    if (lineSpacingMultiply != 0 || lineSpacingAdd != 0) {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
        targetTextView.setLineSpacing(
          lineSpacingAdd != 0
            ? lineSpacingAdd
            : targetTextView.getLineSpacingExtra(),
          lineSpacingMultiply != 0
            ? lineSpacingMultiply
            : targetTextView.getLineSpacingMultiplier()
        );
      } else {
