...
public class CampaignManager {
  ...
  private static final String PERSISTENCE_LOCAL_CAMPAIGNS_FILE_NAME =
    "com.batch.localcampaigns.persist.json";
  ...
  public void updateCampaignList(
    @NonNull List<LocalCampaign> updatedCampaignList
  ) {
    synchronized (this.campaignListLock) {
      ...
      if (Logger.shouldLogForLevel(LoggerLevel.INTERNAL)) {
        Logger.internal(
          TAG,
          "Loaded " + this.campaignList.size() + " campaign(s)"
        );
        String publicToken;
   ...
   */
  public void deleteAllCampaigns(Context context, boolean persist)
   ...
   */
  @VisibleForTesting
  public List<LocalCampaign> cleanCampaignList(
    @NonNull List<LocalCampaign> campaignsToClean
  ) {
    ...
    for (LocalCampaign campaign : campaignsToClean) {
      ...
      if (
        campaign.endDate != null && campaign.endDate.compareTo(currentDate) < 0
      ) {
        Logger.internal(
          TAG,
          "Ignoring campaign " + campaign.id + " since it is past its end_date"
        );
        continue;
      }
      ...
      try {
        ...
        Logger.internal(
          TAG,
          "View tracker is unavailable, campaign " +
          campaign.id +
          " capping can't be evaluated."
        );
      }
      ...
      if (
        ...
        Logger.internal(
          TAG,
          "Campaign " + campaign.id + " is over max API level"
        );
        continue;
      }
   ...
   */
  @VisibleForTesting
  protected boolean isCampaignOverCapping(
    LocalCampaign campaign,
    boolean ignoreMinInterval
    ...
    if (
      ...
      Logger.internal(
        TAG,
        "Campaign's minimum display interval has not been reached"
      );
      return true;
    }
  ...
  protected boolean isCampaignDisplayable(LocalCampaign campaign) {
    ...
    if (
      ...
      campaign.startDate.compareTo(currentDate) > 0
    ) {
      Logger.internal(
        TAG,
        "Ignoring campaign " + campaign.id + " since it has not begun yet"
      );
      return false;
    }
    ...
    try {
      if (isCampaignOverCapping(campaign, false)) {
        Logger.internal(
          ...
          "Campaign " +
          campaign.id +
          " is over capping/minimum display interval."
        );
        return false;
      }
    } catch (ViewTrackerUnavailableException e) {
    ...
    if (
      campaign.minimumAPILevel != null &&
      Parameters.MESSAGING_API_LEVEL < campaign.minimumAPILevel
    ) {
      Logger.internal(
        TAG,
        "Campaign " + campaign.id + " has a minimum API level too high"
      );
      return false;
    }
    ...
    for (LocalCampaign campaign : campaignList) {
      for (LocalCampaign.Trigger trigger : campaign.triggers) {
        if (trigger != null && trigger instanceof EventLocalCampaignTrigger) {
          newWatchedEvents.add(
            ((EventLocalCampaignTrigger) trigger).name.toUpperCase(Locale.US)
          );
        }
      }
    }
  ...
  public void saveCampaigns(
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Can't serialize local campaigns before the save operation",
        e
      );
      e.printStackTrace();
    }
  ...
  public void saveCampaignsAsync(
    @NonNull final Context context,
    @NonNull final List<LocalCampaign> campaigns
  ) {
    TaskExecutorProvider
      .get(context)
      .execute(() -> saveCampaigns(context, campaigns));
  }
  ...
  public void deleteSavedCampaignsAsync(@NonNull final Context context) {
    TaskExecutorProvider
      .get(context)
      .execute(() -> deleteSavedCampaigns(context));
  }

  public boolean hasSavedCampaigns(Context context) {
    try {
      return persistor.hasSavedData(
        context,
        PERSISTENCE_LOCAL_CAMPAIGNS_FILE_NAME
      );
    } catch (PersistenceException e) {
      Logger.internal(
        TAG,
        "Can't determine if there is saved local campaigns",
        e
      );
      return false;
    }
  ...
  public boolean loadSavedCampaignResponse(@NonNull final Context context) {
    ...
    try {
      campaignsRawData =
        persistor.loadData(context, PERSISTENCE_LOCAL_CAMPAIGNS_FILE_NAME);
    } catch (PersistenceException e) {
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "Can't convert json to LocalCampaignsResponse : " + ex.toString()
      );
      return false;
    }
    return true;
