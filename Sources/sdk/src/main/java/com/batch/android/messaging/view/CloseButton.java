...
public class CloseButton extends View implements Styleable
{
    ...
    public void recomputeMetrics()
    {
        ...
        computedGlyphPadding = glyphPadding >= 0 ? glyphPadding : (int) ( UNSCALED_GLYPH_PADDING_PX * density * viewScalingRatio );

        glyphPaint.setStrokeWidth(glyphWidth >= 0 ? glyphWidth : (int) ( UNSCALED_GLYPH_WIDTH_PX * density * viewScalingRatio ));

        borderPaint.setStrokeWidth(Math.max(glyphPaint.getStrokeWidth() + StyleHelper.dpToPixels(
                resources,
                1.0f),
        ...
        countdownOval.set(halfWidth - radius,
                halfHeight - radius,
                halfWidth + radius,
                halfHeight + radius);

        borderOval.set(padding,
                padding,
                getWidth() - padding,
                getHeight() - padding);
    }
        ...
        for (Map.Entry<String, String> rule : rules.entrySet()) {
            if ("background-color".equalsIgnoreCase(rule.getKey())) {
                ...
                try {
                    ...
                    Logger.internal(TAG,
                            "Unparsable glyph color (" + rule.getValue() + ")",
                            e);
                }
