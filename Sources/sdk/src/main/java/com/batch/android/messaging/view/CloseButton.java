...
public class CloseButton extends View implements Styleable {
  ...
  public void init() {
    ...
    setPadding(
      StyleHelper.dpToPixels(getResources(), (float) DEFAULT_PADDING_DP)
    );
  ...
  public void recomputeMetrics() {
    ...
    float viewScalingRatio =
      (getWidth() - padding * 2) / (DEFAULT_SIZE_DP * density);
    computedGlyphPadding =
      glyphPadding >= 0
        ? glyphPadding
        : (int) (UNSCALED_GLYPH_PADDING_PX * density * viewScalingRatio);

    glyphPaint.setStrokeWidth(
      glyphWidth >= 0
        ? glyphWidth
        : (int) (UNSCALED_GLYPH_WIDTH_PX * density * viewScalingRatio)
    );
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (widthMode == MeasureSpec.AT_MOST) {
      width =
        (int) Math.min(DEFAULT_SIZE_DP * density + getPadding() * 2, width);
