...
public class InboxSyncWebserviceClient
  ...
  public void run() {
    try {
      Logger.internal(
        TAG,
        "Starting inbox sync (" + buildURL().toString() + ")"
      );
  ...
  private InboxWebserviceResponse computeResponse(JSONObject json)
    ...
    try {
      ...
      for (int i = 0; i < rawNotifications.length(); i++) {
        ...
        if (rawNotification instanceof JSONObject) {
          String notificationId =
            ((JSONObject) rawNotification).reallyOptString(
                "notificationId",
                null
              );
            ...
            if (isCandidates(notificationId)) {
              ...
              try {
                ...
                if (
                  InboxDatasourceProvider
                    .get(applicationContext)
                    .insert(newNotification, fetcherId)
                ) {
      ...
      r.notifications =
        InboxDatasourceProvider
          .get(applicationContext)
          .getNotifications(ids, fetcherId);
      return r;
