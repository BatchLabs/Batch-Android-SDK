...
public class BatchBannerView
{
    ...
    BatchBannerView(@NonNull BatchMessage rawMessage,
    ...
    public void show(@NonNull Activity activity)
    {
        ...
        if (activity instanceof Batch.Messaging.DisplayHintProvider) {
            Batch.Messaging.DisplayHint hint = ( (Batch.Messaging.DisplayHintProvider) activity ).getBatchMessageDisplayHint(
                    rawMessage);
            if (hint != null) {
                ...
                if (hint.strategy == Batch.Messaging.DisplayHintStrategy.EMBED) {
                    if (hint.view instanceof FrameLayout) {
                        ...
                        Logger.error(MessagingModule.TAG,
                                "Could not embed BatchBannerView, internal error.");
                    }
                    return;
                }
    ...
    public void show(@NonNull final View anchorView)
    {
        ...
        if (shown) {
            Logger.error(MessagingModule.TAG,
                    "This banner has already been shown. Ignoring.");

            return;
        }
        shown = true;

        new Handler(anchorView.getContext().getMainLooper()).post(() -> {
            try {
                LocalBroadcastManagerProvider.get(
                        anchorView.getContext()).sendBroadcast(new Intent(
                        MessagingModule.ACTION_DISMISS_BANNER));
                shownContainer = EmbeddedBannerContainerProvider.get(
                        anchorView,
                        rawMessage,
                        message,
                        analyticsDelegate,
    ...
    public void embed(@NonNull final FrameLayout embedLayout)
    {
        ...
        new Handler(embedLayout.getContext().getMainLooper()).post(() -> {
            try {
                LocalBroadcastManagerProvider.get(
                        embedLayout.getContext()).sendBroadcast(new Intent(
                        MessagingModule.ACTION_DISMISS_BANNER));
                shownContainer = EmbeddedBannerContainerProvider.get(
                        embedLayout,
                        rawMessage,
