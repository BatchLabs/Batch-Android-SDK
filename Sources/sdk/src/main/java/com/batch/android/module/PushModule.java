...
public class PushModule extends BatchModule {
   ...
   */
  public void setNotificationInterceptor(
  ...
  public Registration getRegistration(@NonNull Context context) {
    try {
      ...
      if (parameters == null) {
        Logger.internal(
          TAG,
          "Could not fetch registration: failed to read parameters"
        );
        return null;
      }

      final String registrationID = parameters.get(
        ParameterKeys.PUSH_REGISTRATION_ID_KEY
      );
      if (registrationID == null) {
        return null;
      }

      String registrationProvider = parameters.get(
      ...
      String senderID = parameters.get(
  ...
  public EnumSet<PushNotificationType> getNotificationsType(Context context) {
    ...
    if (tempNotifType != null) {
      ...
      param =
        ParametersProvider.get(context).get(ParameterKeys.PUSH_NOTIF_TYPE);
    }
  ...
  public void setNotificationsType(EnumSet<PushNotificationType> types) {
    if (types == null) {
      Logger.error(
        TAG,
        "Call to setNotificationsType with null type given, aborting"
      );
      return;
    }
  ...
  public void appendBatchData(Intent pushIntent, Intent openIntent) {
    try {
      ...
      Logger.error(
        TAG,
        "Error while appending Batch data to open intent : " +
  ...
  public void appendBatchData(Bundle pushIntentExtras, Intent openIntent) {
    try {
      InternalPushData data = InternalPushData.getPushDataForReceiverBundle(
        pushIntentExtras
      );
      if (data == null) {
        Logger.error(
   ...
   */
  public void appendBatchData(
    @NonNull RemoteMessage remoteMessage,
    Intent openIntent
  ) {
    Bundle extras = BatchPushHelper.firebaseMessageToReceiverBundle(
      remoteMessage
    );
    if (extras == null) {
  ...
  public void displayNotification(
    ...
    try {
      ...
      Logger.internal(
        TAG,
        "An error occured while handling push notification",
        e
      );
      Logger.error(
        TAG,
        "An error occured during display : " + e.getLocalizedMessage()
      );
    }
  }
  ...
  public void displayNotification(
    ...
    try {
      ...
      Logger.error(
        TAG,
        "An error occurred during display : " + e.getLocalizedMessage()
      );
    }
  }
  ...
  public void onNotificationDisplayed(Context context, Intent intent) {
    try {
      if (shouldDisplayPush(context, intent)) {
        InternalPushData pushData = InternalPushData.getPushDataForReceiverIntent(
          intent
        );
  ...
  public void onNotificationDisplayed(Context context, RemoteMessage message) {
    try {
      if (shouldDisplayPush(context, message)) {
        InternalPushData pushData = InternalPushData.getPushDataForFirebaseMessage(
          message
        );
        BatchPushHelper.markPushAsShown(context, pushData.getPushId());
  ...
  private String getAppVersion() {
    try {
      PackageInfo packageInfo = RuntimeManagerProvider
        ...
        .getPackageManager()
        .getPackageInfo(
          RuntimeManagerProvider.get().getContext().getPackageName(),
          0
        );
  ...
  private void requestRegistration(
    @NonNull final PushRegistrationProvider provider
  ) {
  ...
  private void emitRegistration(
    ...
    i.putExtra(
      Batch.EXTRA_REGISTRATION_IDENTIFIER,
    ...
    RuntimeManagerProvider
      ...
      .run(state -> {
        if (state != State.OFF) {
          ...
          if (parameters != null) {
            String currentRegistrationID = parameters.get(
              ParameterKeys.PUSH_REGISTRATION_ID_KEY
            );
            String currentRegistrationProvider = parameters.get(
              ParameterKeys.PUSH_REGISTRATION_PROVIDER_KEY
            );
            String currentSenderID = parameters.get(
              ParameterKeys.PUSH_REGISTRATION_SENDERID_KEY
            );

            parameters.set(
              ParameterKeys.PUSH_APP_VERSION_KEY,
              getAppVersion(),
              true
            );
            ...
            if (
              ...
              WebserviceLauncher.launchPushWebservice(
  ...
  public static boolean isBatchPushServiceAvailable() {
    try {
      final PackageManager packageManager = RuntimeManagerProvider
        .get()
        .getContext()
        .getPackageManager();
      final Intent intent = new Intent(
        RuntimeManagerProvider.get().getContext(),
        BatchPushService.class
      );
    ...
    if (!didSetupRegistrationProvider) {
      ...
      if (context != null) {
        ...
        registrationProvider =
          new PushRegistrationProviderFactory(
            context,
        ...
        Logger.internal(
          TAG,
          "No context set, cannot try to instantiate a registration provider. Will retry."
        );
      }
    }
