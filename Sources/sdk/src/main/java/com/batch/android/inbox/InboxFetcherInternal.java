...
public class InboxFetcherInternal
{
    ...
    private Executor fetchExecutor = Executors.newSingleThreadExecutor(new NamedThreadFactory(
            "inbox.fetcher"));
    ...
    private InboxFetcherInternal(@NonNull TrackerModule trackerModule,
     ...
     */
    @Provide
    public static InboxFetcherInternal provide(@NonNull Context context,
                                               String installID,
                                               boolean useCache)
    {
        if (useCache) {
    ...
    {
        if (l == null) {
            l = new BatchInboxFetcher.OnNewNotificationsFetchedListener()
            {
                @Override
        ...
        InboxWebserviceListener wsClientListener = new InboxWebserviceListener()
        {
            ...
            {
                Logger.internal(TAG,
                        "Inbox fetch success (new notifications) ----\n" + response.toString());
                try {
                    List<InboxNotificationContentInternal> addedNotifications = handleFetchSuccess(
                            response,
                            true);
                    userListener.onFetchSuccess(convertInternalModelsToPublic(addedNotifications),
    ...
    public void fetchNextPage(BatchInboxFetcher.OnNextPageFetchedListener listener)
    {
        ...
        if (listener == null) {
            listener = new BatchInboxFetcher.OnNextPageFetchedListener()
            {
                @Override
                public void onFetchSuccess(@NonNull List<BatchInboxNotificationContent> notifications,
                                           boolean endReached)
            ...
            public void onSuccess(InboxWebserviceResponse response)
            {
                ...
                try {
                    ...
                    List<InboxNotificationContentInternal> addedNotifications = handleFetchSuccess(
                            response,
                            cursor == null);
                    finalListener.onFetchSuccess(convertInternalModelsToPublic(
                            addedNotifications),
                            !response.hasMore);
                } catch (ResultHandlingError e) {
                    Logger.internal(TAG, "Failed to handle inbox fetch response", e);
    ...
    private void fetch(@Nullable final String cursor,
                       @NonNull final InboxWebserviceListener wsClientListener)
    {
        ...
        if (this.fetcherType == FetcherType.USER_IDENTIFIER) {
            ...
            if (TextUtils.isEmpty(this.authKey)) {
                wsClientListener.onFailure(
                        "Inbox API Error: Authentication Key can't be null or empty in user mode");
                return;
            }
        }
        ...
        fetchExecutor.execute(() -> {
            ...
            try {
                ...
                new InboxFetchWebserviceClient(c,
                        fetcherType,
                        identifier,
                        authKey,
                        maxPageSize,
    ...
    private boolean sync(@Nullable final String cursor,
                         final InboxWebserviceListener wsClientListener)
    {
        if (datasource != null && fetcherId != -1) {
            ...
            if (!candidates.isEmpty()) {
                fetchExecutor.execute(() -> {
                    ...
                    try {
                        // No need for the TaskExecutor, run the WS directly on this thread since it has to work serially
                        new InboxSyncWebserviceClient(c,
                                fetcherType,
                                identifier,
                                authKey,
                                maxPageSize,
    ...
    private List<InboxNotificationContentInternal> handleFetchSuccess(InboxWebserviceResponse response,
                                                                      boolean askedForNewNotifications) throws ResultHandlingError
    {
            ...
            for (InboxNotificationContentInternal respNotif : response.notifications) {
                ...
                if (duplicateNotif != null) {
                    ...
                    if (respNotif.identifiers.identifier.equals(duplicateNotif.identifiers.identifier)) {
                        Logger.internal(TAG,
                                "InboxFetcher: Got the exact same notification twice, skipping. " + respNotif.identifiers.identifier);
                    } else {
                        Logger.internal(TAG,
                                "Merging notifications for sendID " + sendID + " (identifiers: " + respNotif.identifiers.identifier + ", " + duplicateNotif.identifiers.identifier + ")");
