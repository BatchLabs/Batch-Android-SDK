...
public class OptOutModule extends BatchModule {
  ...
  public static final String INTENT_OPTED_OUT =
    Parameters.LIBRARY_BUNDLE + ".optout.enabled";

  public static final String INTENT_OPTED_IN =
    Parameters.LIBRARY_BUNDLE + ".optout.disabled";
  ...
  private static final String OPTED_OUT_FROM_BATCHSDK_KEY =
    "app.batch.opted_out";

  private static final String SHOULD_SEND_OPTIN_EVENT_KEY =
    "app.batch.send_optin_event";

  private static final String MANIFEST_OPT_OUT_BY_DEFAULT_KEY =
    ...
    if (preferences == null) {
      preferences =
        context
          ...
          .getSharedPreferences(OPT_OUT_PREFERENCES_NAME, Context.MODE_PRIVATE);
    }
    return preferences;
  }
  ...
  public boolean isOptedOutSync(Context context) {
    if (isOptedOut == null) {
      ...
      if (prefs.contains(OPTED_OUT_FROM_BATCHSDK_KEY)) {
        ...
        isOptedOut =
          getManifestBoolean(context, MANIFEST_OPT_OUT_BY_DEFAULT_KEY, false);
        prefs
          .edit()
          .putBoolean(OPTED_OUT_FROM_BATCHSDK_KEY, isOptedOut)
          .apply();
        if (isOptedOut) {
          Logger.info(
            TAG,
  ...
  public void trackOptinEventIfNeeded(
    @NonNull Context context,
    @NonNull AdvertisingID advertisingID
  ) {
    SharedPreferences prefs = getPreferences(context);
  ...
  public void optIn(Context context) {
    ...
    if (oldOptOutValue) {
      ...
      LocalBroadcastManagerProvider
        .get(context)
        .sendBroadcast(new Intent(INTENT_OPTED_IN));
    }
  }
        ...
        eventPromise =
          TrackerModuleProvider
            ...
            .trackOptOutEvent(
              context,
              advertisingID,
              InternalEvents.OPT_OUT_AND_WIPE_DATA
            );
      } else {
        eventPromise =
          TrackerModuleProvider
            .get()
            .trackOptOutEvent(context, advertisingID, InternalEvents.OPT_OUT);
      }
        ...
        new Handler(context.getMainLooper())
          .post(() -> {
            if (listener != null) {
              if (
                listener.onError() ==
                BatchOptOutResultListener.ErrorPolicy.CANCEL
              ) {
  ...
  private void doOptOut(Context context, boolean wipeData) {
    ...
    getPreferences(context)
      .edit()
      .putBoolean(OPTED_OUT_FROM_BATCHSDK_KEY, true)
      .apply();
  ...
  private boolean getManifestBoolean(
    Context context,
    String key,
    boolean fallback
  ) {
