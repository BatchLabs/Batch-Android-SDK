...
public class OptOutModule extends BatchModule
{
    ...
    private final static String OPTED_OUT_FROM_BATCHSDK_KEY = "app.batch.opted_out";

    private final static String SHOULD_SEND_OPTIN_EVENT_KEY = "app.batch.send_optin_event";

    private final static String MANIFEST_OPT_OUT_BY_DEFAULT_KEY = "batch_opted_out_by_default";
    ...
    public void optIn(Context context)
    {
        ...
        if (oldOptOutValue) {
            ...
            getPreferences(context).edit()
                    ...
                    .apply();
            LocalBroadcastManagerProvider.get(context).sendBroadcast(new Intent(
                    INTENT_OPTED_IN));
        }
    }

    public Promise<Void> optOut(final Context context,
                                ...
                                final BatchOptOutResultListener listener)
    {
        ...
        if (!oldOptOutValue) {
            ...
            if (wipeData) {
                eventPromise = TrackerModuleProvider.get().trackOptOutEvent(context,
                        advertisingID,
                        InternalEvents.OPT_OUT_AND_WIPE_DATA);
            } else {
                eventPromise = TrackerModuleProvider.get().trackOptOutEvent(context,
                        advertisingID,
                        InternalEvents.OPT_OUT);
            }
            ...
            eventPromise.then(value -> new Handler(context.getMainLooper()).post(() -> {
                if (listener != null) {
                    listener.onSuccess();
                }
            ...
            eventPromise.catchException(e -> new Handler(context.getMainLooper()).post(() -> {
                if (listener != null) {
